classdef AutoDX < handle
    ...
    %   [DFDX,DX] = AutoDX.GetOptimalDX(F, T, X, DX_MAX, IX, ORDER)
    ...
    methods
        ...
        function [dFdX_out, dX_out, dXmax_out, err_out, fcnerr_out] = ...
                GetOptimalDX(obj, F, t0, X0, dX_max, order, dFdX_known, varargin)
            ...
            for iX = numX:1
                [dFdX_out(:,iX), dX_out(:,iX), dXmax_out(:,iX), err_out(:,iX), fcnerr_out(:,iX)] = ...
                    obj.AutoDX_iX(F, t0, X0, dX_max, iX, order, dFdX_known, args{:});
            end
        ...
        function [dFdX_out, dX_out, dXmax_out, err_out, fcnerr_out] = ...
                AutoDX_iX(obj, F, t0, X0, dX_max, iX, order, dFdX_known, varargin)
            ...
            for j = 1:numdX_exp
                ...
                i_dX = obj.advance(i_dX, -(npoints-1), 1, np);
                i_dX_prev = obj.advance(i_dX, -1, 1, np);
                i_dX_next = obj.advance(i_dX, 1, 1, np);
                ...
                for currF = numF:1
                    ...
                    if(errt_valid(currF) && (abs(slope_errt) > order*slope_errt_endtol))
                        ...
                        [err_cond, err_sub] = obj.getErrorCoeff(order, F0(currF), Fpert);
                        err_sub = eps*err_sub;
                        ...
                        if(Cn_true(currF) == 0.0)
                            ...
                            err_c = order*abs(Cn_true(currF))*dX_vec(i_dX_opt)^(order+1) - err_sub;
                        end
                        ...
                        err_out(currF) = (err_c + err_sub)/dX_vec(i_dX_opt) + abs(Cn_true(currF))*dX_vec(i_dX_opt)^order;
                ...
                i_dX = obj.advance(i_dX, npoints-1, 1, np); % Reset the 'current' point
        ...
        %%
        function iout = advance(obj, i, di, imin, imax)
        ...
        %%
        function [dFdX, Fpert] ...
                = getGradient_Order(obj, F, t, X, iX, dX, order, FX, varargin)
