function [y,H,R] = rrdot3D(t,x,sig)
%RRDOT3D Range and range-rate measurement model for 3-D inertial state
%   [y,H] = rrdot3D(t,x,sig) returns the measurement, y, and measurement
% partials, H, for 2-way range and range-rate from an artificial ground
% station located at [rp,0,0], where rp, the radius of the
% earth, and the earth rotation rate (omega in the code) are based on the
% parameters used in the NASA GSFC Orbit Determination Error Analysis System. 
% If the measurement noise standard deviation matrix, sig, is ommited, a
% default value of sig = diag([1e-3 1e-6]) is used.
...
%   [y,H,R] = rrdot3D(t,x,sig) also outputs measurement noise covariance
% matrix.
%
...
y = NaN(2,lent);
k = (dot(dr,rs)>=0);
y(:,k) = 2*[rho(k); dot(dr(:,k),dv(:,k))./rho(:,k)];
udr = unit(dr);
if nargout > 1,
    Hvv = shiftdim(udr,-1);
    Hrr = shiftdim(udr,-1);
