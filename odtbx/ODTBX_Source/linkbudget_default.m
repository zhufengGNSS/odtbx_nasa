function [linkbudget] = linkbudget_default(linkbudget, fieldname, default_val)
    ...
    if nargin < 2
        error('ODTBX:linkbudget_default:NotEnoughInputs','Not enough input arguments.');
    end
    if nargin < 3
        default = [];
    end
    ...
    if ~isempty(linkbudget) && ~isa(linkbudget,'struct')
        error('ODTBX:linkbudget_default:Arg1Notlinkbudgetstruct',...
    ...
    if ~isfield(linkbudget, fieldname) || isempty(linkbudget.(fieldname))
        ...
        warning('ODTBX:linkbudget_default:UsingDefault', '%s not set, setting default value of %s.', fieldname, defaultname);
    end
end