function [h] = plot_gps_pp_gpsyaw(phys_param)
% Plots the modeled GPS yaw angle physical parameter from gps_phys_params vs time.
%
%   INPUTS
...
h = figure;
plot((phys_param.epoch-phys_param.epoch(1))*24,phys_param.GPS_yaw,'.');
ylabel('GPS Yaw Angle (deg)');

ts = cell(2,1);
ts{1} = sprintf('Yaw Angle for GPS SV PRN %d',phys_param.meta.TX_ID.GPS_PRN);
