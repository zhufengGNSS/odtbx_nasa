function [h] = plot_gps_prange(gps_data, PRNs)
% Plots the raw GPS pseudorange vs time.
%
% Plots the GPS pseudorange for each GPS PRN specified in the argument.
% Multiple PRNs are plotted on subplots, but four or less PRN subplots are
% recommended for readability.  If there is no data for that PRN then there
% will be an empty subplot.
%
%   INPUTS
...
for i = 1:sp
    ...
    if i <= length(prns)
        plot((gps_data.PRN_data{datind(i)}.epoch-baseepoch)*24,gps_data.PRN_data{datind(i)}.pseudorange,'.');
    end
    yl{1} = 'P-range (km)';
    if i <= length(prns)
    ...
    if i == 1 && ~isempty(prns)
        ts = cell(2,1);
        if sp < 2
            ts{1} = sprintf('Pseudorange for GPS SV PRN(s): %s',num2str(sort(PRNs)));
        else
            ts{1} = sprintf('Pseudorange for GPS SV PRN(s): [%s]',num2str(sort(PRNs)));
        end
