...
function [failed] = estbat_tspan_test
    ...
    % Run the estimator over the timespan of 0 to 300 but let estbat figure
    % out the time steps from integ:
    tspan = [0 300];
    ...
    opts = setOdtbxOptions('MonteCarloCases',30,'UpdateIterations',6);
    ...
    for(a=1:length(t))
        ...
        if(any(dt))
            failed = 1;
            fprintf(2, '\n *** estbat_tspan_test: t test failed.\n');
        end
    end
    ...
    for(a=1:length(xhat))
        ...
        if(max(max(dx)) > dx_max)
            failed = 1;
            disp(max(max(dx)))
            fprintf(2, '\n *** estbat_tspan_test: xhat test failed.\n');
        end
    end
    ...
    for(a=1:length(Phat))
        ...
        if(max(max(dP)) > dP_max)
            failed = 1;
            disp(max(max(dP)))
            fprintf(2, '\n *** estbat_tspan_test: Phat test failed.\n');
        end
    end
    ...
    for(a=1:length(e))
        ...
        if(max(max(de)) > de_max)
            failed = 1;
            disp(max(max(de)))
            fprintf(2, '\n *** estbat_tspan_test: e test failed.\n');
        end
    end
    ...
    for(a=1:length(y))
        ...
        if(max(max(dy)) > dy_max)
            failed = 1;
            disp(max(max(dy)))
            fprintf(2, '\n *** estbat_tspan_test: y test failed.\n');
        end
    end
    
    if((max(max(max(abs(Pa - Pa_b))))) > eps)
        failed = 1;
        fprintf(2, '\n *** estbat_tspan_test: Pa test failed.\n');
    end
    
    if((max(max(max(abs(Pv - Pv_b))))) > eps)
        failed = 1;
        fprintf(2, '\n *** estbat_tspan_test: Pv test failed.\n');
    end
    
    if((max(max(max(abs(Pw - Pw_b))))) > eps)
        failed = 1;
        fprintf(2, '\n *** estbat_tspan_test: Pw test failed.\n');
    end
    
    if((max(max(max(abs(Phata - Phata_b))))) > eps)
        failed = 1;
        fprintf(2, '\n *** estbat_tspan_test: Phata test failed.\n');
    end
    
    if((max(max(max(abs(Phatv - Phatv_b))))) > eps)
        failed = 1;
        fprintf(2, '\n *** estbat_tspan_test: Phatv test failed.\n');
    end
    
    if((max(max(max(abs(Phatw - Phatw_b))))) > eps)
        failed = 1;
        fprintf(2, '\n *** estbat_tspan_test: Phatw test failed.\n');
    end
