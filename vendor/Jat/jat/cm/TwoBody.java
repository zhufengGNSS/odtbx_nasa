 ...
 * @version 1.0
 */
public class TwoBody implements Derivatives, SecondDerivatives
{
	...
	public TwoBody()
	{
		...
		double e0 = this.eccentricAnomaly();
		this.ma = e0 - this.e * Math.sin(e0);
	...
	public void setRV(VectorN r, VectorN v){
		rv2Elements(r, v);
		double e0 = this.eccentricAnomaly();
		this.ma = e0 - this.e * Math.sin(e0);
	...
	public double eccentricAnomaly()
	{
		...
		double den = 1.0 + e * cta;
		double cose = (e + cta) / den;
		double sine = sta*Math.sqrt(1.0 - e*e) / den;
		double e0 = Math.atan2(sine, cose);
		return e0;
	}
...
//		if (Math.abs(ecc) < 0.0)
//		{
//			System.err.println("Error: Trying to solve Kepler's equation for negative eccentricity");
//			System.exit(-99);
//		}
//		// Elliptical case
//		if ((ecc > 0.0)&& (ecc < 1.0)){
//			// set initial guess
////			if ((mean_anomaly > Math.PI)||((mean_anomaly > -1.0*Math.PI)&&(mean_anomaly < 0.0))){
////				ea = mean_anomaly - ecc;
////			}
////			else {
////				ea = mean_anomaly + ecc;
...
//			while ((it < maxit) && (Math.abs(de) > 1.0E-16))
//			{
...
//		}
//		// parabolic case not handled so crash, Sorry!
//		if (ecc == 1.0) {
//			System.err.println("Error: Trying to solve Kepler's equation for parabolic orbit");
//			System.exit(-99);
//		}		
//		// Hyperbolic case
//		if (ecc > 1.0) {
...
//			}
//			else {
//				if ((ecc < 3.6)&&(Math.abs(mean_anomaly)>Math.PI)){
//					ea = mean_anomaly - ecc*Math.signum(mean_anomaly);
//				}
//				else {
//					ea = mean_anomaly/(ecc - 1.0);
//				}				
//			}
//			// Newton-Raphson iteration
//			while ((it < maxit) && (Math.abs(de) > 1.0E-10))
//			{
//				de = (mean_anomaly - ecc*Math.sinh(ea) + ea) / (ecc * Math.cosh(ea) - 1.0);
//				ea = ea + de;
//				it = it + 1;
//			}
//			
...
//}
	 ...
	 * @return Eccentric anomaly in radians.
	 */
	public double solveKepler(){
		return solveKepler(this.meanAnomaly(), this.eccentricity());
	}
...
//		double q = Math.sqrt((1.0 + this.e) / (1.0 - this.e));
	...
	public void step(double dt){
		...
		this.ma = this.ma + this.meanMotion() * dt;
		double ea = TwoBody.solveKepler(ma, this.e);
		...
		this.rv = new VectorN(this.randv());
	}
...
//		double E0 = this.eccentricAnomaly();
//		this.ma = this.ma + this.meanMotion() * dt;
//		double E = TwoBody.solveKepler(ma, this.e);
//		
//		double deltaE = E - E0;
//
//		double sinE = Math.sin(deltaE);
//		double cosE = Math.cos(deltaE);
//		double omcosE = 1.0 - cosE;
//		VectorN r0 = this.getR();
//		VectorN v0 = this.getV();
//		double r0mag = r0.mag();
//		double sqrtmu = Math.sqrt(this.mu);
//		double sqrta = Math.sqrt(this.a);
//		double sigma0 = (r0.dotProduct(v0))/sqrtmu;
//		
//		double F = 1.0 - this.a*omcosE/r0mag;
//		double G = this.a * sigma0*omcosE/sqrtmu + r0mag*sqrta*sinE/sqrtmu;
//		
//		VectorN r = (r0.times(F)).plus(v0.times(G));
//		double rmag = r.mag();
//		
//		double Ft = -1.0*(sqrtmu*sqrta*sinE)/(rmag*r0mag);
//		double Gt = 1.0 - this.a*omcosE/rmag;
//		VectorN v = (r0.times(Ft)).plus(v0.times(Gt));
//		this.setRV(r, v);
//	}
	...
	public double[] derivs(double t, double[] y, double[] ydot)
	{
