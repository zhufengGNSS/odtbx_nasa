...
static integer c_b210 = 100000;
static integer c_b212 = 2000000;
...
/* Main program */ MAIN__(void)
{
    ...
    extern /* Subroutine */ int errch_(char *, char *, ftnlen, ftnlen), 
	    rdnbl_(char *, char *, logical *, ftnlen, ftnlen), bodn2c_(char *,
	     integer *, logical *, ftnlen), swapd_(doublereal *, doublereal *)
	    ;
    static char utext[80*21];
    extern /* Subroutine */ int dafus_(doublereal *, integer *, integer *, 
	    doublereal *, integer *), objcf2_(I_fp, integer *, integer *, 
	    integer *, integer *), objcf1_(I_fp, integer *, integer *, 
	    integer *), rndem_(char *, logical *, integer *, integer *, 
    ...
    static doublereal filwin[1006], intval;
    ...
    static doublereal tmpwin[1006], segsum[6];
    ...
    extern /* Subroutine */ int objcmp_(integer *), objset_(integer *, 
	    ...
	    logical *, integer *, char *, integer *, doublereal *, char *, 
	    char *, ftnlen, ftnlen, ftnlen, ftnlen), scardc_(integer *, char *
	    , ftnlen), scardi_(integer *, integer *), objmod_(integer *, 
...
/*     of taking apart the the format and constructing appropriate */
/*     names. */
...
/*     Help text. */
    ...
    s_copy(htext + 2160, "      -g           display summary grouped by cove"
	    "rage", (ftnlen)80, (ftnlen)54);
    s_copy(htext + 2240, "      -n           display bodies/frames using onl"
	    "y numeric id-codes", (ftnlen)80, (ftnlen)68);
    s_copy(htext + 2320, "      -o           display summary ordered by body"
	    "/frame name", (ftnlen)80, (ftnlen)61);
    s_copy(htext + 2400, " ", (ftnlen)80, (ftnlen)1);
    s_copy(htext + 2480, "      -[bod]       display summary for body [bod]", 
	    (ftnlen)80, (ftnlen)49);
    s_copy(htext + 2560, "      -sb[bod]     display summary for body [bod]", 
	    (ftnlen)80, (ftnlen)49);
    s_copy(htext + 2640, "      -sc[cen]     display summary for center of m"
	    "otion/relative-to frame [cen]", (ftnlen)80, (ftnlen)79);
    s_copy(htext + 2720, "      -at [time]   display summary if coverage con"
	    "tains epoch [time]", (ftnlen)80, (ftnlen)68);
    s_copy(htext + 2800, "      -from [beg]  display summary if coverage con"
	    "tains interval [beg]:[end]", (ftnlen)80, (ftnlen)76);
    s_copy(htext + 2880, "      -to [end]    display summary if coverage con"
	    "tains interval [beg]:[end]", (ftnlen)80, (ftnlen)76);
    s_copy(htext + 2960, " ", (ftnlen)80, (ftnlen)1);
    s_copy(htext + 3040, "      -f [list]    summarize kernels listed in the"
	    " [list] file", (ftnlen)80, (ftnlen)62);
    s_copy(htext + 3120, " ", (ftnlen)80, (ftnlen)1);
    s_copy(htext + 3200, "      -h           display help", (ftnlen)80, (
	    ftnlen)31);
    s_copy(htext + 3280, "      -v           display version", (ftnlen)80, (
	    ftnlen)34);
    s_copy(htext + 3360, " ", (ftnlen)80, (ftnlen)1);
    s_copy(htext + 3440, "   An LSK file must be provided on the command lin"
	    "e to display times in", (ftnlen)80, (ftnlen)71);
    s_copy(htext + 3520, "   UTC formats. FK file(s) must be provided on the"
	    " command line to", (ftnlen)80, (ftnlen)66);
    s_copy(htext + 3600, "   display names of any frames that are not built "
	    "into the Toolkit.", (ftnlen)80, (ftnlen)67);
    s_copy(htext + 3680, " ", (ftnlen)80, (ftnlen)1);
    s_copy(htext + 3760, "   See the BRIEF User's Guide for more information."
	    , (ftnlen)80, (ftnlen)51);
    s_copy(htext + 3840, " ", (ftnlen)80, (ftnlen)1);
...
/*     Construct and display program version. */
    ...
    suffix_("3.0.0, January 14, 2008         ", &c__1, line, (ftnlen)32, (
	    ftnlen)80);
...
/*     If command line is blank, display usage and stop. */
	...
	for (i__ = 1; i__ <= 21; ++i__) {
	    writit_(utext + ((i__1 = i__ - 1) < 21 && 0 <= i__1 ? i__1 : 
		    s_rnge("utext", i__1, "brief_", (ftnlen)916)) * 80, (
		    ftnlen)80);
	}
	s_stop("", (ftnlen)0);
    }
...
/*     If help display was requsted, display help text and stop. */
	...
	for (i__ = 1; i__ <= 49; ++i__) {
	    writit_(htext + ((i__1 = i__ - 1) < 49 && 0 <= i__1 ? i__1 : 
		    s_rnge("htext", i__1, "brief_", (ftnlen)1566)) * 80, (
		    ftnlen)80);
	}
	s_stop("", (ftnlen)0);
    }
...
/*     eqivalent to "-at". "-at" takes precedence over any combination */
/*     of "-from" and "-to". */
...
/*     array used to hold the "formats" for the names of objects. */
    ...
    s_copy(fmtpic, frmat + (((i__1 = num + (c__ + (o + (t << 1) << 1) << 1) - 
	    ...
	    ftnlen)1639)) << 5), (ftnlen)32, (ftnlen)32);

/*     Initialize the cells. */

    ssizec_(&c_b210, winsym, (ftnlen)64);
    ssizei_(&c_b210, winptr);
    ssized_(&c_b212, winval);
    ssized_(&c__1000, filwin);
    ssized_(&c__2, segwin);
    ssized_(&c__1000, tmpwin);
    objinl_(&objsiz, &c_b210, objlis);
...
/*                 non-inertial we never go back to inertial. */
		    ...
		    object[(i__1 = objsiz - 1) < 3 && 0 <= i__1 ? i__1 : 
			    s_rnge("object", i__1, "brief_", (ftnlen)1866)] = 
			    class__;
...
/*              time and body have occured and finish "No data ..." */
/*              reporting. */
...
/*              the window table. */
		...
		disply_(fmtpic, &tdsp, &gdsp, &obnam, objlis, winsym, winptr, 
			winval, timtyp, kertyp, (ftnlen)32, (ftnlen)64, (
			ftnlen)32, (ftnlen)80);
...
/*        display what we have. */
	...
	disply_(fmtpic, &tdsp, &gdsp, &obnam, objlis, winsym, winptr, winval, 
		timtyp, kertyp, (ftnlen)32, (ftnlen)64, (ftnlen)32, (ftnlen)
		80);
    }
...
/*     Miscelaneous functions that don't deserve to have their own */
/*     source files. */
...
L_idmch:
    ret_val = object[0] <= ref[(i__1 = object[4] - 1) < 2 && 0 <= i__1 ? i__1 
	    : s_rnge("ref", i__1, "mchboc_", (ftnlen)2596)] && object[1] >= 
	    ref[(i__2 = object[4] - 1) < 2 && 0 <= i__2 ? i__2 : s_rnge("ref",
	     i__2, "mchboc_", (ftnlen)2596)];
    return ret_val;
