	...
	chval_len)
{
    ...
    char frkfn[255], value[12], lskfn[255];
    logical exist;
    extern integer rtrim_(char *, ftnlen);
    extern /* Subroutine */ int ljust_(char *, char *, ftnlen, ftnlen), 
	    bodn2c_(char *, integer *, logical *, ftnlen), str2et_(char *, 
	    doublereal *, ftnlen);
    integer handle;
    ...
    extern /* Subroutine */ int setmsg_(char *, ftnlen), sigerr_(char *, 
	    ...
	    *, char *, ftnlen, ftnlen, ftnlen, ftnlen), convrt_(doublereal *, 
	    ...
	    getfat_(char *, char *, char *, ftnlen, ftnlen, ftnlen), pcklof_(
	    char *, integer *, ftnlen);
...
/*        then the error 'SPICE(NOINPUTFILENAME)' will be signalled. */

/*     2) If nput file name is blank, then the error */
/*        'SPICE(BLANKINPUTFILENAME)' will be signalled. */

/*     3) If nput file does not exist, then the error */
/*        'SPICE(INPUTFILENOTEXIST)' will be signalled. */

/*     4) If output SPK file name was not provided neither on */
/*        the command line nor as a value of the setup file keyword, */
/*        then the error 'SPICE(NOOUTPUTFILENAME)' will be signalled. */

/*     5) If output file name is blank, then the error */
/*        'SPICE(BLANKOUTPTFILENAME)' will be signalled. */

/*     6) If append flag specified in the setup file keyword */
/*        is not recognizedthe the error 'SPICE(UNRECOGNAPPFLAG)' */
/*        will be signalled. */
...
/*        is not recongnized, then the error 'SPICE(UNRECOGNDATATYPE)' */
/*        will be signalled. */
...
/*        'SPICE(NOFRAMENAME)' will be signalled. */
...
/*        'SPICE(NOLINESPERRECCOUNT)' will be signalled. */
...
/*        'SPICE(BADLINEPERRECCOUNT)' will be signalled. */
...
/*        keyword is not recongnized, then the error */
/*        'SPICE(UNRECOGNPRECTYPE)' will be signalled. */
...
/*        'SPICE(NOTAPCKFILE)' will be signalled. */

/*     31) If input flag is not recognized, then the error */
/*        'SPICE(MKSPKBUGSETUP1)', 'SPICE(MKSPKBUGSETUP2)', */
/*        'SPICE(MKSPKBUGSETUP3)' or 'SPICE(MKSPKBUGSETUP4)' */
/*        will be signalled. */
...
/*     Input and output filenames length. */
...
/*        Retrive TIME_WRAPPER value. Parameter KTIMWR declared in */
/*        include file. */
...
/*        Retrive DATA_ORDER value. Parameter KDATOR declared */
/*        in include file. */
...
/*        Retrive DATA_DELIMITER value and check it. Parameter KDATDL */
...
/*           Delimiter is not allowed. Error will be signaled. */
	    ...
	    setmsg_("Delimiter '#' specified in the setup file keyword '#' i"
		    "s not recongnized. Refer to the User's Guide for the pro"
		    "gram for the list of supported input data delimiters.", (
		    ftnlen)164);
...
/*     FLAG        is the string containig text identifier for the */
/*                 keyword of interest. */
...
/*     Standard SPICE error handling. */
    ...
    if (s_cmp(flag__, "OBJECT ID", flag_len, (ftnlen)9) == 0) {
...
/*        Retrive OUTPUT_SPK_TYPE  value and check it. Parameter KOSPTP */
/*        declared in include file. */
...
/*        Retrive IGNORE_FIRST_LINES value.Parameter KIGNFL declared */
/*        in include file. */
...
/*        Retrive LINES_PER_RECORD value. Parameter KLNPRC declared */
/*        in include file. */
...
/*        Retrive EPOCH_STR_LENGTH value. Parameter KELENG declared */
/*        in include file. */
...
/*        Retrive POLYNOM_DEGREE value. Parameter KPLDEG declared */
/*        in include file. */
...
/*           Coefficient that converts distance unit to KM. */
	    ...
	    convrt_(&c_b240, dstunt, "KM", dpval, (ftnlen)80, (ftnlen)2);
	} else {
	    *found = FALSE_;
	}
...
/*           Coefficient that converts angle unit to radian. */
	    ...
	    convrt_(&c_b240, angunt, "RADIANS", dpval, (ftnlen)80, (ftnlen)7);
	} else {
	    *found = FALSE_;
	}
	...
	if (s_cmp(prctvl, "NO PRECESSION", rtrim_(prctvl, (ftnlen)80), (
	    ...
	    *dpval = 0.;
	    ...
	    setmsg_("Precession type '#' specified in the setup file keyword"
		    " '#' is not recongnized. Refer to the User's Guide for t"
		    "he program for the list of supported precession types.", (
		    ftnlen)165);
...
/*        in include file. */
    ...
    return 0;
/* $Procedure   SETUPA ( Get LSK and PCK filenames and load its content ) */
...
/*     filenames but loads corresponding files using the pool or */
/*     binary loaders. */
