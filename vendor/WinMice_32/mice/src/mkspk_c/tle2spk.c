	...
	frnmvl_len, ftnlen sgidvl_len)
{
    ...
    doublereal begint, epochs[5001], geophs[8], covpad;
    logical fmodel;
    integer iorder[5000], ordvec[5000];
    char geolst[255];
    logical eof;
    extern /* Subroutine */ int setmsg_(char *, ftnlen), errint_(char *, 
	    integer *, ftnlen), sigerr_(char *, ftnlen), namfrm_(char *, 
    ...
    extern /* Subroutine */ int tostdo_(char *, ftnlen), cltext_(char *, 
...
/*     The number of seconds for which propagation will be allowed */
/*     before the first TLE epoch and after the last TLE epoch, */
/*     nominally set to one half day. */
...
/*        description of the error. */
	...
	zzgetelm_(&c__1950, lines, &epochs[(i__1 = i__ - 1) < 5001 && 0 <= 
		i__1 ? i__1 : s_rnge("epochs", i__1, "tle2spk_", (ftnlen)624)]
		, &elems[(i__2 = j - 1) < 50010 && 0 <= i__2 ? i__2 : s_rnge(
		"elems", i__2, "tle2spk_", (ftnlen)624)], &ok, error + 512, (
...
/*           at the end of the array. */
	    ...
	    ordvec[(i__1 = put - 1) < 5000 && 0 <= i__1 ? i__1 : s_rnge("ord"
		    "vec", i__1, "tle2spk_", (ftnlen)663)] = iorder[0];
	    for (k = 2; k <= 5000; ++k) {
		if (epochs[(i__2 = ordvec[(i__1 = put - 1) < 5000 && 0 <= 
			i__1 ? i__1 : s_rnge("ordvec", i__1, "tle2spk_", (
			ftnlen)667)] - 1) < 5001 && 0 <= i__2 ? i__2 : s_rnge(
			"epochs", i__2, "tle2spk_", (ftnlen)667)] == epochs[(
			i__4 = iorder[(i__3 = k - 1) < 5000 && 0 <= i__3 ? 
			i__3 : s_rnge("iorder", i__3, "tle2spk_", (ftnlen)667)
			] - 1) < 5001 && 0 <= i__4 ? i__4 : s_rnge("epochs", 
			i__4, "tle2spk_", (ftnlen)667)]) {
		    ordvec[(i__1 = trash - 1) < 5000 && 0 <= i__1 ? i__1 : 
			    s_rnge("ordvec", i__1, "tle2spk_", (ftnlen)669)] =
			     iorder[(i__2 = k - 1) < 5000 && 0 <= i__2 ? i__2 
			    : s_rnge("iorder", i__2, "tle2spk_", (ftnlen)669)]
			    ;
		    --trash;
		} else {
		    ++put;
		    ordvec[(i__1 = put - 1) < 5000 && 0 <= i__1 ? i__1 : 
			    s_rnge("ordvec", i__1, "tle2spk_", (ftnlen)673)] =
			     iorder[(i__2 = k - 1) < 5000 && 0 <= i__2 ? i__2 
			    : s_rnge("iorder", i__2, "tle2spk_", (ftnlen)673)]
			    ;
		}
	    }
...
/*           Assign the final epoch time. */
	    ...
	    endt = epochs[(i__1 = put - 1) < 5001 && 0 <= i__1 ? i__1 : 
		    s_rnge("epochs", i__1, "tle2spk_", (ftnlen)696)];
...
/*           correspondingly. */
	    ...
	    epochs[0] = epochs[(i__1 = put - 1) < 5001 && 0 <= i__1 ? i__1 : 
		    s_rnge("epochs", i__1, "tle2spk_", (ftnlen)715)];
	    for (i__ = 1; i__ <= 10; ++i__) {
		elems[(i__1 = i__ - 1) < 50010 && 0 <= i__1 ? i__1 : s_rnge(
			"elems", i__1, "tle2spk_", (ftnlen)718)] = elems[(
...
/*        at the end of the array. */
	...
	ordvec[(i__1 = put - 1) < 5000 && 0 <= i__1 ? i__1 : s_rnge("ordvec", 
		i__1, "tle2spk_", (ftnlen)756)] = iorder[0];
	i__1 = i__;
	for (k = 2; k <= i__1; ++k) {
	    if (epochs[(i__3 = ordvec[(i__2 = put - 1) < 5000 && 0 <= i__2 ? 
		    i__2 : s_rnge("ordvec", i__2, "tle2spk_", (ftnlen)760)] - 
		    1) < 5001 && 0 <= i__3 ? i__3 : s_rnge("epochs", i__3, 
		    "tle2spk_", (ftnlen)760)] == epochs[(i__5 = iorder[(i__4 =
		     k - 1) < 5000 && 0 <= i__4 ? i__4 : s_rnge("iorder", 
		    i__4, "tle2spk_", (ftnlen)760)] - 1) < 5001 && 0 <= i__5 ?
		     i__5 : s_rnge("epochs", i__5, "tle2spk_", (ftnlen)760)]) 
		    {
		ordvec[(i__2 = trash - 1) < 5000 && 0 <= i__2 ? i__2 : s_rnge(
			"ordvec", i__2, "tle2spk_", (ftnlen)762)] = iorder[(
			i__3 = k - 1) < 5000 && 0 <= i__3 ? i__3 : s_rnge(
			"iorder", i__3, "tle2spk_", (ftnlen)762)];
		--trash;
	    } else {
		++put;
		ordvec[(i__2 = put - 1) < 5000 && 0 <= i__2 ? i__2 : s_rnge(
			"ordvec", i__2, "tle2spk_", (ftnlen)766)] = iorder[(
			i__3 = k - 1) < 5000 && 0 <= i__3 ? i__3 : s_rnge(
			"iorder", i__3, "tle2spk_", (ftnlen)766)];
	    }
	}
...
/*        the last epoch by COVPAD seconds. */

	endt = epochs[(i__1 = put - 1) < 5001 && 0 <= i__1 ? i__1 : s_rnge(
		"epochs", i__1, "tle2spk_", (ftnlen)803)] + covpad;
	tostdo_(" ", (ftnlen)1);
	...
	endt = epochs[(i__1 = put - 1) < 5001 && 0 <= i__1 ? i__1 : s_rnge(
		"epochs", i__1, "tle2spk_", (ftnlen)823)] + covpad;
	tostdo_(" ", (ftnlen)1);
	tostdo_("Writing  SPK segment...", (ftnlen)23);
	spkw10_(handle, &object, cnidvl, frnmvl, &begint, &endt, sgidvl, 
		geophs, &put, elems, epochs, frnmvl_len, sgidvl_len);
