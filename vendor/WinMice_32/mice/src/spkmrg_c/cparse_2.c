...
/*     In order to specify the minimum and maximum occurance of keywords */
...
/*        NUMMIN  INT   Minimum occurance of chlid keyword in CHLNUM. */

/*        NUMMAX  INT   Maximum occurance of chlid keyword in CHLNUM. */
...
/*        Have we already come across this keyword? */
	...
	if (p == 0) {
	    ...
	    s_copy(kwnam + (((i__2 = nkwd - 1) < 100 && 0 <= i__2 ? i__2 : 
		    s_rnge("kwnam", i__2, "cparse_", (ftnlen)677)) << 5), kwd,
		     (ftnlen)32, (ftnlen)32);
	}
...
/*        Check its children. */
	...
	sygetc_(kwd, kwdsym, kwdptr, kwdval, &nchl, values, &fnd, (ftnlen)32, 
		kwdsym_len, kwdval_len, (ftnlen)127);
	i__2 = nchl;
	for (j = 1; j <= i__2; ++j) {
	    nextwd_(values + ((i__3 = j - 1) < 40 && 0 <= i__3 ? i__3 : 
		    s_rnge("values", i__3, "cparse_", (ftnlen)687)) * 127, 
		    kwd, values + ((i__4 = j - 1) < 40 && 0 <= i__4 ? i__4 : 
		    s_rnge("values", i__4, "cparse_", (ftnlen)687)) * 127, (
		    ftnlen)127, (ftnlen)32, (ftnlen)127);
	    ucase_(kwd, kwd, (ftnlen)32, (ftnlen)32);
...
/*           If this is a new keyword, add it to our array. */
		...
		s_copy(kwnam + (((i__3 = nkwd - 1) < 100 && 0 <= i__3 ? i__3 :
			 s_rnge("kwnam", i__3, "cparse_", (ftnlen)698)) << 5),
...
/*     Now, for each keyword in KWNAM find the child keywords, if any. */
	...
	s_copy(kwd, kwnam + (((i__2 = i__ - 1) < 100 && 0 <= i__2 ? i__2 : 
		s_rnge("kwnam", i__2, "cparse_", (ftnlen)717)) << 5), (ftnlen)
		32, (ftnlen)32);
	sygetc_(kwd, kwdsym, kwdptr, kwdval, &nchl, values, &fnd, (ftnlen)32, 
		kwdsym_len, kwdval_len, (ftnlen)127);
	p = chlptr[(i__2 = i__ - 1) < 101 && 0 <= i__2 ? i__2 : s_rnge("chlp"
		"tr", i__2, "cparse_", (ftnlen)721)];
	if (fnd) {
	    i__2 = nchl;
	    for (j = 1; j <= i__2; ++j) {
		nextwd_(values + ((i__3 = j - 1) < 40 && 0 <= i__3 ? i__3 : 
			s_rnge("values", i__3, "cparse_", (ftnlen)726)) * 127,
			 kwd, values + ((i__4 = j - 1) < 40 && 0 <= i__4 ? 
			i__4 : s_rnge("values", i__4, "cparse_", (ftnlen)726))
			 * 127, (ftnlen)127, (ftnlen)32, (ftnlen)127);
		nextwd_(values + ((i__3 = j - 1) < 40 && 0 <= i__3 ? i__3 : 
			s_rnge("values", i__3, "cparse_", (ftnlen)727)) * 127,
			 minch, values + ((i__4 = j - 1) < 40 && 0 <= i__4 ? 
			i__4 : s_rnge("values", i__4, "cparse_", (ftnlen)727))
			 * 127, (ftnlen)127, (ftnlen)20, (ftnlen)127);
		nextwd_(values + ((i__3 = j - 1) < 40 && 0 <= i__3 ? i__3 : 
			s_rnge("values", i__3, "cparse_", (ftnlen)728)) * 127,
			 maxch, values + ((i__4 = j - 1) < 40 && 0 <= i__4 ? 
			i__4 : s_rnge("values", i__4, "cparse_", (ftnlen)728))
			 * 127, (ftnlen)127, (ftnlen)20, (ftnlen)127);
		ucase_(kwd, kwd, (ftnlen)32, (ftnlen)32);
...
/*     If the parser is not initialized an error will be signalled. Other */
...
/*     '\begin_commads' through the end of the file. The following */
/*     code fragment illustrates how one perform this task. */

/*        CALL RDTEXT ('INPUT.CMD', LINE, EOF) */

/*        DO WHILE (LINE .NE. '\begin_commads') */
...
/*           it. */
	    ...
	    s_copy(ln, line, (ftnlen)127, line_len);
	    replch_(ln, "\t", " ", ln, (ftnlen)127, (ftnlen)1, (ftnlen)1, (
		    ftnlen)127);
	    ljust_(ln, ln, (ftnlen)127, (ftnlen)127);

/*           A blank line or a comment line is of no use to EVALCP. */

	    if (s_cmp(ln, " ", (ftnlen)127, (ftnlen)1) == 0 || *(unsigned 
		    char *)ln == ';') {
...
/*              the line plus one (makes for easier parsing). */
		...
		ep = i_indx(ln, "=", (ftnlen)127, (ftnlen)1);
		cp = i_indx(ln, ";", (ftnlen)127, (ftnlen)1);
		if (cp == 0) {
		    cp = lastnb_(ln, (ftnlen)127) + 1;
		}
...
/*              command, but an error will be signalled when we return */
/*              to PARSE later. */

		if (ep >= 1 && ep < cp) {
		    s_copy(action, "EVAL", (ftnlen)10, (ftnlen)4);
		} else {
		    i__ = frstnb_(ln, (ftnlen)127);
		    vlen = vlen + cp - i__ + 1;
		    if (vlen > 300) {
...
/*           decrease STKLEV until we find a place for this keyword. */
	    ...
	    i__6 = chlptr[(i__2 = kwstk[(i__1 = newlev) < 6 && 0 <= i__1 ? 
		    i__1 : s_rnge("kwstk", i__1, "cparse_", (ftnlen)1127)]) < 
		    101 && 0 <= i__2 ? i__2 : s_rnge("chlptr", i__2, "cparse_"
		    , (ftnlen)1127)] - chlptr[(i__3 = kwstk[(i__1 = newlev) < 
		    6 && 0 <= i__1 ? i__1 : s_rnge("kwstk", i__1, "cparse_", (
		    ftnlen)1127)] - 1) < 101 && 0 <= i__3 ? i__3 : s_rnge(
		    "chlptr", i__3, "cparse_", (ftnlen)1127)];
	    p = isrchi_(&kwdnum, &i__6, &chlnum[(i__5 = chlptr[(i__4 = kwstk[(
		    i__1 = newlev) < 6 && 0 <= i__1 ? i__1 : s_rnge("kwstk", 
		    i__1, "cparse_", (ftnlen)1127)] - 1) < 101 && 0 <= i__4 ? 
		    i__4 : s_rnge("chlptr", i__4, "cparse_", (ftnlen)1127)]) <
		     100 && 0 <= i__5 ? i__5 : s_rnge("chlnum", i__5, "cpars"
		    "e_", (ftnlen)1127)]);
	    while(newlev != 0 && p == 0) {
		--newlev;
		i__6 = chlptr[(i__2 = kwstk[(i__1 = newlev) < 6 && 0 <= i__1 ?
			 i__1 : s_rnge("kwstk", i__1, "cparse_", (ftnlen)1132)
			]) < 101 && 0 <= i__2 ? i__2 : s_rnge("chlptr", i__2, 
			"cparse_", (ftnlen)1132)] - chlptr[(i__3 = kwstk[(
			i__1 = newlev) < 6 && 0 <= i__1 ? i__1 : s_rnge("kws"
			"tk", i__1, "cparse_", (ftnlen)1132)] - 1) < 101 && 0 
			<= i__3 ? i__3 : s_rnge("chlptr", i__3, "cparse_", (
			ftnlen)1132)];
		p = isrchi_(&kwdnum, &i__6, &chlnum[(i__5 = chlptr[(i__4 = 
			kwstk[(i__1 = newlev) < 6 && 0 <= i__1 ? i__1 : 
			s_rnge("kwstk", i__1, "cparse_", (ftnlen)1132)] - 1) <
			 101 && 0 <= i__4 ? i__4 : s_rnge("chlptr", i__4, 
			"cparse_", (ftnlen)1132)]) < 100 && 0 <= i__5 ? i__5 :
			 s_rnge("chlnum", i__5, "cparse_", (ftnlen)1132)]);
	    }
...
/*              where this keyword may appear. */
		...
		for (i__ = 1; i__ <= i__1; ++i__) {
			...
			suffix_(kwnam + (((i__2 = i__ - 1) < 100 && 0 <= i__2 
				...
				ftnlen)1154)) << 5), &c__1, msg, (ftnlen)32, (
				ftnlen)320);
		    }
		}
		if (k == 1) {
...
/*           maximum occurance? */
	    ...
	    j = chlptr[(i__5 = kwstk[(i__1 = i__) < 6 && 0 <= i__1 ? i__1 : 
		    s_rnge("kwstk", i__1, "cparse_", (ftnlen)1234)] - 1) < 
		    101 && 0 <= i__5 ? i__5 : s_rnge("chlptr", i__5, "cparse_"
		    , (ftnlen)1234)];
	    if (kwcntr[(i__1 = p + newlev * 40 - 1) < 240 && 0 <= i__1 ? i__1 
		    : s_rnge("kwcntr", i__1, "cparse_", (ftnlen)1236)] == 
		    nummax[(i__5 = p + j - 1) < 100 && 0 <= i__5 ? i__5 : 
		    s_rnge("nummax", i__5, "cparse_", (ftnlen)1236)]) {
		if (nummax[(i__1 = p + j - 1) < 100 && 0 <= i__1 ? i__1 : 
			s_rnge("nummax", i__1, "cparse_", (ftnlen)1238)] > 1) 
			{
		...
		inttxt_(&nummax[(i__1 = p + j - 1) < 100 && 0 <= i__1 ? i__1 :
			 s_rnge("nummax", i__1, "cparse_", (ftnlen)1244)], 
			maxch, (ftnlen)20);
		...
		if (newlev > 0) {
		    ...
		    repmc_(msg, "%", kwnam + (((i__5 = kwstk[(i__1 = newlev) <
			    ...
			    "cparse_", (ftnlen)1252)] - 1) < 100 && 0 <= i__5 
			    ? i__5 : s_rnge("kwnam", i__5, "cparse_", (ftnlen)
			    1252)) << 5), msg, (ftnlen)320, (ftnlen)1, (
			    ftnlen)32, (ftnlen)320);
		} else {
		    suffix_(".", &c__0, msg, (ftnlen)1, (ftnlen)320);
		}
...
/*        given for parent keyword index I. */
	...
	j = chlptr[(i__2 = kwstk[(i__5 = i__) < 6 && 0 <= i__5 ? i__5 : 
		s_rnge("kwstk", i__5, "cparse_", (ftnlen)1318)] - 1) < 101 && 
		0 <= i__2 ? i__2 : s_rnge("chlptr", i__2, "cparse_", (ftnlen)
		1318)];
	i__4 = chlptr[(i__2 = kwstk[(i__5 = i__) < 6 && 0 <= i__5 ? i__5 : 
		s_rnge("kwstk", i__5, "cparse_", (ftnlen)1320)]) < 101 && 0 <=
		 i__2 ? i__2 : s_rnge("chlptr", i__2, "cparse_", (ftnlen)1320)
		] - chlptr[(i__3 = kwstk[(i__5 = i__) < 6 && 0 <= i__5 ? i__5 
		: s_rnge("kwstk", i__5, "cparse_", (ftnlen)1320)] - 1) < 101 
		&& 0 <= i__3 ? i__3 : s_rnge("chlptr", i__3, "cparse_", (
		ftnlen)1320)];
	for (k = 1; k <= i__4; ++k) {
	    if (kwcntr[(i__5 = k + i__ * 40 - 1) < 240 && 0 <= i__5 ? i__5 : 
		    s_rnge("kwcntr", i__5, "cparse_", (ftnlen)1322)] < nummin[
		    (i__2 = k + j - 1) < 100 && 0 <= i__2 ? i__2 : s_rnge(
		    "nummin", i__2, "cparse_", (ftnlen)1322)]) {
		inttxt_(&nummin[(i__5 = k + j - 1) < 100 && 0 <= i__5 ? i__5 :
			 s_rnge("nummin", i__5, "cparse_", (ftnlen)1324)], 
			minch, (ftnlen)20);
		if (i__ > 0) {
		...
		if (nummax[(i__5 = k + j - 1) < 100 && 0 <= i__5 ? i__5 : 
			s_rnge("nummax", i__5, "cparse_", (ftnlen)1332)] == 
			nummin[(i__2 = k + j - 1) < 100 && 0 <= i__2 ? i__2 : 
			s_rnge("nummin", i__2, "cparse_", (ftnlen)1332)]) {
		...
		repmc_(msg, "%", kwnam + (((i__2 = chlnum[(i__5 = k + j - 1) <
			...
			"cparse_", (ftnlen)1341)] - 1) < 100 && 0 <= i__2 ? 
			i__2 : s_rnge("kwnam", i__2, "cparse_", (ftnlen)1341))
			 << 5), msg, (ftnlen)320, (ftnlen)1, (ftnlen)32, (
			ftnlen)320);
		if (nummin[(i__5 = k + j - 1) < 100 && 0 <= i__5 ? i__5 : 
			s_rnge("nummin", i__5, "cparse_", (ftnlen)1343)] == 1)
			 {
		...
		if (i__ > 0) {
		    ...
		    repmc_(msg, "%", kwnam + (((i__2 = kwstk[(i__5 = i__) < 6 
			    ...
			    "rse_", (ftnlen)1351)] - 1) < 100 && 0 <= i__2 ? 
			    i__2 : s_rnge("kwnam", i__2, "cparse_", (ftnlen)
			    1351)) << 5), msg, (ftnlen)320, (ftnlen)1, (
			    ftnlen)32, (ftnlen)320);
		    lstcmd = linstk[(i__5 = i__ - 1) < 5 && 0 <= i__5 ? i__5 :
			     s_rnge("linstk", i__5, "cparse_", (ftnlen)1353)];
		} else {
		    suffix_(".", &c__0, msg, (ftnlen)1, (ftnlen)320);
		}
...
/*     REASON     is the reason a parser error occured. REASON may */
