...
/* Main program */ MAIN__(void)
{
    ...
    logical same;
    char line[80];
    integer body[1000], nval;
    doublereal tvec[6], wind[4006];
    logical used;
    integer nsrc, nspk, unit, work[1000];
    char text[80];
    integer i__, j, k;
    ...
    doublereal endtim;
    char begsym[100];
    doublereal locval[6006];
    integer cmdptr[506];
    doublereal locwin[4006];
    char cmdsym[100*506];
    doublereal segwin[4006];
    integer newhan;
    doublereal winval[6006];
    integer ngbody;
    logical commnt;
    doublereal segsum[5];
    char symbol[100], locsym[16*1006], tstamp[22];
    integer locptr[1006], cmtunt, logunt;
    extern /* Subroutine */ int trcoff_(void), erract_(char *, char *, ftnlen,
	     ftnlen);
    extern /* Character */ VOID crtptr_(char *, ftnlen, char *, integer *, 
	    char *, ftnlen, ftnlen);
    char srcsym[100];
    doublereal tmpwin[4006];
    extern /* Subroutine */ int ssizec_(integer *, char *, ftnlen);
    integer winptr[1006];
    extern /* Subroutine */ int ssizei_(integer *, integer *);
    char winsym[16*1006];
    extern /* Subroutine */ int ssized_(integer *, doublereal *), cputim_(
	    doublereal *), tkvrsn_(char *, char *, ftnlen, ftnlen), tostdo_(
	    char *, ftnlen), prompt_(char *, char *, ftnlen, ftnlen), sygetc_(
	    char *, char *, integer *, char *, integer *, char *, logical *, 
	    ftnlen, ftnlen, ftnlen, ftnlen), setmsg_(char *, ftnlen), sigerr_(
	    char *, ftnlen), chkout_(char *, ftnlen), byebye_(char *, ftnlen),
	     furnsh_(char *, ftnlen), spkopn_(char *, char *, integer *, 
	    integer *, ftnlen, ftnlen), txtopn_(char *, integer *, ftnlen), 
	    txtops_(integer *), writln_(char *, integer *, ftnlen), wrdnln_(
	    char *, char *, integer *, integer *, ftnlen, ftnlen), scardi_(
	    integer *, integer *), wninsd_(doublereal *, doublereal *, 
	    doublereal *), lparsm_(char *, char *, integer *, integer *, char 
	    *, ftnlen, ftnlen, ftnlen), nparsi_(char *, integer *, char *, 
	    integer *, ftnlen, ftnlen), dafopr_(char *, integer *, ftnlen), 
	    wnintd_(doublereal *, doublereal *, doublereal *), errint_(char *,
	     integer *, ftnlen), intstr_(integer *, char *, ftnlen), sygetd_(
	    char *, char *, integer *, doublereal *, integer *, doublereal *, 
	    logical *, ftnlen, ftnlen), wndifd_(doublereal *, doublereal *, 
	    doublereal *), spksub_(integer *, doublereal *, char *, 
	    doublereal *, doublereal *, integer *, ftnlen), wnunid_(
	    doublereal *, doublereal *, doublereal *), syputd_(char *, 
	    doublereal *, integer *, char *, integer *, doublereal *, ftnlen, 
	    ftnlen);
    integer bod;
    extern /* Subroutine */ int suffix_(char *, integer *, char *, ftnlen, 
	    ftnlen), txtopr_(char *, integer *, ftnlen);
    logical fnd, all, log__;
    char val[200*500], err[80], tkv[8];
...
/*        interpretting the values assigned to the WORK array. */
...
/*     Initialize all cells. */
    ...
    ssizec_(&c__500, cmdsym, (ftnlen)100);
    ssizei_(&c__500, cmdptr);
    ssizec_(&c__500, cmdval, (ftnlen)200);
    ssizec_(&c__1000, winsym, (ftnlen)16);
    ssizei_(&c__1000, winptr);
    ssized_(&c__6000, winval);
    ssizec_(&c__1000, locsym, (ftnlen)16);
    ssizei_(&c__1000, locptr);
    ssized_(&c__6000, locval);
    ssized_(&c__4000, segwin);
    ssized_(&c__4000, addwin);
    ssized_(&c__4000, tmpwin);
    ssized_(&c__4000, locwin);
    ssized_(&c__4000, wind);
    ssized_(&c__4000, gwind);

/*     Initialize the WORK array.  Set all the values to UNUSED. */

    for (i__ = 1; i__ <= 1000; ++i__) {
	work[(i__1 = i__ - 1) < 1000 && 0 <= i__1 ? i__1 : s_rnge("work", 
		i__1, "spkmerge_", (ftnlen)691)] = -1;
    }
...
/*               1234567890123456789012 */
    ...
    repmc_(text, "#", "3.3", text, (ftnlen)80, (ftnlen)1, (ftnlen)3, (ftnlen)
	    80);
    repmc_(text, "#", tkv, text, (ftnlen)80, (ftnlen)1, (ftnlen)8, (ftnlen)80)
	    ;
    tostdo_(text, (ftnlen)80);
    tostdo_(" ", (ftnlen)1);
...
/*     command file to process. */
	...
	tostdo_(" ", (ftnlen)1);
	prompt_("> ", line, (ftnlen)2, (ftnlen)80);
	tostdo_(" ", (ftnlen)1);
    }
...
/*     command file symbol table. */
    ...
    sygetc_("SPK_KERNEL", cmdsym, cmdptr, cmdval, &nval, val, &fnd, (ftnlen)
	    10, (ftnlen)100, (ftnlen)200, (ftnlen)200);
    if (! fnd) {
	chkin_("SPKMERGE", (ftnlen)8);
	setmsg_("This error is never supposed to occur. Please contact NAIF.",
		 (ftnlen)59);
...
/*        Open the new file that we are about to create. */
	...
	spkopn_(val + ((i__1 = nspk - 1) < 500 && 0 <= i__1 ? i__1 : s_rnge(
		"val", i__1, "spkmerge_", (ftnlen)801)) * 200, "SPKMERGE", &
		c__0, &newhan, (ftnlen)200, (ftnlen)8);
...
/* Writing concatenation */
	...
	i__3[1] = rtrim_(val + ((i__2 = nspk - 1) < 500 && 0 <= i__2 ? i__2 : 
		s_rnge("val", i__2, "spkmerge_", (ftnlen)813)) * 200, (ftnlen)
		200), a__1[1] = val + ((i__1 = nspk - 1) < 500 && 0 <= i__1 ? 
		i__1 : s_rnge("val", i__1, "spkmerge_", (ftnlen)813)) * 200;
	s_cat(text, a__1, i__3, &c__2, (ftnlen)80);
	tostdo_(text, (ftnlen)80);
...
/*        Retrieve the name of the log file. */
	...
	sygetc_(symbol, cmdsym, cmdptr, cmdval, &n, value, &fnd, (ftnlen)100, 
		(ftnlen)100, (ftnlen)200, (ftnlen)200);
...
/*           If so, create the new log file. */
	    ...
	    txtopn_(value, &logunt, (ftnlen)200);
...
/*           created. */
	    ...
	    log__ = TRUE_;
	    s_copy(name__, value, (ftnlen)200, (ftnlen)200);
	} else {
...
/*        SPK file we're creating: VAL(NSPK). */
	...
	s_copy(line, "; $ LOG FILE", (ftnlen)80, (ftnlen)12);
	repmc_(line, "$", val + ((i__1 = nspk - 1) < 500 && 0 <= i__1 ? i__1 :
		 s_rnge("val", i__1, "spkmerge_", (ftnlen)871)) * 200, line, (
		ftnlen)80, (ftnlen)1, (ftnlen)200, (ftnlen)80);
	writln_(line, &logunt, (ftnlen)80);
	writln_(" ", &logunt, (ftnlen)1);

/*        Add the time and date of creation. */

	s_copy(line, "; Created $.", (ftnlen)80, (ftnlen)12);
	repmc_(line, "$", tstamp, line, (ftnlen)80, (ftnlen)1, (ftnlen)22, (
		ftnlen)80);
	writln_(line, &logunt, (ftnlen)80);
	writln_(";", &logunt, (ftnlen)1);
...
/*        Write the name of the SPK file that is being created. */
	...
	wrdnln_("SPK_KERNEL", val + ((i__1 = nspk - 1) < 500 && 0 <= i__1 ? 
		i__1 : s_rnge("val", i__1, "spkmerge_", (ftnlen)902)) * 200, &
		c__1, &logunt, (ftnlen)10, (ftnlen)200);
...
/*        If we find some, then write their names in the log file. */
	...
	if (fnd) {
		...
		wrdnln_("INCLUDE_TEXT_FILE", val + ((i__2 = i__ - 1) < 500 && 
			0 <= i__2 ? i__2 : s_rnge("val", i__2, "spkmerge_", (
			ftnlen)928)) * 200, &c__3, &logunt, (ftnlen)17, (
			ftnlen)200);
	    }
	}
...
/*              Convert the BEGIN_TIME value into ET. */
		...
		str2et_(val + ((i__2 = i__ - 1) < 500 && 0 <= i__2 ? i__2 : 
			s_rnge("val", i__2, "spkmerge_", (ftnlen)970)) * 200, 
			&begtim, (ftnlen)200);
...
/*              Fetch the END_TIME value from the command symbol table. */
		...
		sygetc_(symbol, cmdsym, cmdptr, cmdval, &n, value, &fnd, (
			ftnlen)100, (ftnlen)100, (ftnlen)200, (ftnlen)200);
		if (! fnd) {
		    chkin_("SPKMERGE", (ftnlen)8);
...
/*              Convert the END_TIME string to ET. */
		...
		str2et_(value, &endtim, (ftnlen)200);
...
/*           Parse the bodies character string. */
	    ...
	    lparsm_(value, ", ", &c__1000, &ngbody, val, (ftnlen)200, (ftnlen)
		    2, (ftnlen)200);
	    i__1 = ngbody;
	    for (i__ = 1; i__ <= i__1; ++i__) {
		nparsi_(val + ((i__2 = i__ - 1) < 500 && 0 <= i__2 ? i__2 : 
			s_rnge("val", i__2, "spkmerge_", (ftnlen)1042)) * 200,
			 &gbody[(i__4 = i__ - 1) < 1000 && 0 <= i__4 ? i__4 : 
			s_rnge("gbody", i__4, "spkmerge_", (ftnlen)1042)], 
			err, &pnter, (ftnlen)200, (ftnlen)80);
	    }
...
/*           Store the name of the source file in NAME. */
	    ...
	    s_copy(name__, val + ((i__1 = nsrc - 1) < 500 && 0 <= i__1 ? i__1 
		    : s_rnge("val", i__1, "spkmerge_", (ftnlen)1090)) * 200, (
		    ftnlen)200, (ftnlen)200);

/*           Open the SPK file. */

	    dafopr_(name__, &oldhan, (ftnlen)200);
...
/*           Now fetch the values from the command symbol table. */
	    ...
	    sygetc_(begsym, cmdsym, cmdptr, cmdval, &nval, val, &fnd, (ftnlen)
		    100, (ftnlen)100, (ftnlen)200, (ftnlen)200);
...
/*                 Convert the start time string into ET. */
		    ...
		    str2et_(val + ((i__2 = i__ - 1) < 500 && 0 <= i__2 ? i__2 
			    : s_rnge("val", i__2, "spkmerge_", (ftnlen)1139)) 
			    * 200, &begtim, (ftnlen)200);
...
/*           to process is stored in the variable NBODY. */
	    ...
	    if (fnd) {
		lparsm_(value, ", ", &c__1000, &nbody, val, (ftnlen)200, (
			ftnlen)2, (ftnlen)200);
		i__1 = nbody;
		for (i__ = 1; i__ <= i__1; ++i__) {
		    nparsi_(val + ((i__2 = i__ - 1) < 500 && 0 <= i__2 ? i__2 
			    : s_rnge("val", i__2, "spkmerge_", (ftnlen)1198)) 
			    * 200, &body[(i__4 = i__ - 1) < 1000 && 0 <= i__4 
			    ? i__4 : s_rnge("body", i__4, "spkmerge_", (
			    ftnlen)1198)], err, &pnter, (ftnlen)200, (ftnlen)
			    80);
		}
...
/*           Loop over all bodies listed and set WORK to UNUSED. */
	    ...
	    i__1 = nbody;
	    for (i__ = 1; i__ <= i__1; ++i__) {
		work[(i__2 = i__ - 1) < 1000 && 0 <= i__2 ? i__2 : s_rnge(
			"work", i__2, "spkmerge_", (ftnlen)1230)] = -1;
	    }
...
/*                       Store the current body in the BODY array. */
			    ...
			    body[(i__1 = j - 1) < 1000 && 0 <= i__1 ? i__1 : 
...
/*                    SPKMERGE can process. */
			    ...
			    setmsg_("The number of bodies to subset from sou"
				    ...
				    (ftnlen)118);
			    errch_("#", name__, (ftnlen)1, (ftnlen)200);
...
/*                 the following filter was performed on the the ADD */
...
/*                    data for the BODY from this source SPK file. */
			...
			work[(i__1 = j - 1) < 1000 && 0 <= i__1 ? i__1 : 
...
/*                    window. */
			...
			for (i__ = 1; i__ <= i__1; i__ += 2) {
			    spksub_(&oldhan, segsum, segnam, &addwin[(i__2 = 
				    i__ + 5) < 4006 && 0 <= i__2 ? i__2 : 
				    s_rnge("addwin", i__2, "spkmerge_", (
				    ftnlen)1429)], &addwin[(i__4 = i__ + 6) < 
				    4006 && 0 <= i__4 ? i__4 : s_rnge("addwin"
				    , i__4, "spkmerge_", (ftnlen)1429)], &
				    newhan, (ftnlen)40);
			}
...
/*              Retrieve the previous segement from this source SPK file. */
...
/*              contributed data to the new SPK file. */
		...
		while(i__ <= nbody && work[(i__1 = i__ - 1) < 1000 && 0 <= 
...
/*                    new SPK from this source file. */
			...
			if (work[(i__2 = j - 1) < 1000 && 0 <= i__2 ? i__2 : 
...
/*                       current body.  Place them into SEGWIN. */
			    ...
			    intstr_(&body[(i__2 = j - 1) < 1000 && 0 <= i__2 ?
				     i__2 : s_rnge("body", i__2, "spkmerge_", 
				    (ftnlen)1622)], bodch, (ftnlen)16);
...
/*                          windows match. */
				...
				for (k = 1; k <= i__2; ++k) {
				    same = same && segwin[(i__4 = k + 5) < 
					    4006 && 0 <= i__4 ? i__4 : s_rnge(
					    "segwin", i__4, "spkmerge_", (
					    ftnlen)1640)] == tmpwin[(i__5 = k 
					    + 5) < 4006 && 0 <= i__5 ? i__5 : 
					    s_rnge("tmpwin", i__5, "spkmerge_"
					    , (ftnlen)1640)];
				}
			    } else {
				same = FALSE_;
...
/*                 Build up the lines of text for the log file. */
		    ...
		    s_copy(value, " ", (ftnlen)200, (ftnlen)1);
...
/*                       finished logging coverage for this body. */
			    ...
			    work[(i__2 = j - 1) < 1000 && 0 <= i__2 ? i__2 : 
				    s_rnge("work", i__2, "spkmerge_", (ftnlen)
				    1684)] = -1;
			    suffix_(" #,", &c__0, value, (ftnlen)3, (ftnlen)
				    200);
			    repmi_(value, "#", &body[(i__2 = j - 1) < 1000 && 
				    0 <= i__2 ? i__2 : s_rnge("body", i__2, 
				    "spkmerge_", (ftnlen)1687)], value, (
...
/*                 LOG are in UTC. */
		    ...
		    i__1 = n;
		    for (j = 1; j <= i__1; j += 2) {
			et2utc_(&tmpwin[(i__2 = j + 5) < 4006 && 0 <= i__2 ? 
				i__2 : s_rnge("tmpwin", i__2, "spkmerge_", (
				ftnlen)1715)], "C", &c__3, value, (ftnlen)1, (
				ftnlen)200);
			wrdnln_("BEGIN_TIME", value, &c__5, &logunt, (ftnlen)
				10, (ftnlen)200);
			et2utc_(&tmpwin[(i__2 = j + 6) < 4006 && 0 <= i__2 ? 
				i__2 : s_rnge("tmpwin", i__2, "spkmerge_", (
				ftnlen)1718)], "C", &c__3, value, (ftnlen)1, (
				ftnlen)200);
...
/*        Add the include files to the comment area. */
	...
	if (fnd) {
	    i__1 = nval;
	    for (i__ = 1; i__ <= i__1; ++i__) {
		txtopr_(val + ((i__2 = i__ - 1) < 500 && 0 <= i__2 ? i__2 : 
			s_rnge("val", i__2, "spkmerge_", (ftnlen)1766)) * 200,
			 &unit, (ftnlen)200);
...
/*        Next SPK file. */
	...
	sygetc_("SPK_KERNEL", cmdsym, cmdptr, cmdval, &nval, val, &fnd, (
		ftnlen)10, (ftnlen)100, (ftnlen)200, (ftnlen)200);
    }
