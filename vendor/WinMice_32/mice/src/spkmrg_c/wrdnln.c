...
/*      LLMAX     P    Maximum length of line. */
...
/*     LLMAX      is the maximum length of the line. This value should */
...
/*     This routine will try to form the best looking assingment, taking */
...
/*        Put the DO WHILE constuct back in and replace WRITE */
...
/*     have to let it extend past LL, but not past LLMAX. */
...
/* Computing MAX */
    ...
    vp1 = vp;
    ep = vp - 2;
    s_copy(l, " ", (ftnlen)128, (ftnlen)1);
    s_copy(l + (kp - 1), k, 128 - (kp - 1), k_len);
    s_copy(l + (ep - 1), "=", 128 - (ep - 1), (ftnlen)1);

/*     Same thing over again, exept we have no keyword, or equal sign. */
...
/*        If E extends past VLMAX, we'd better do something about it. */
	...
	s_copy(l + (vp - 1), v + (b - 1), 128 - (vp - 1), e - (b - 1));
	writln_(l, unit, (ftnlen)128);
	i__2 = e + 1;
	i__1 = min(i__2,vlmax) - 1;
	b = e + ltrim_(v + i__1, v_len - i__1);
	i__1 = b + 79 - vp1 + 1;
