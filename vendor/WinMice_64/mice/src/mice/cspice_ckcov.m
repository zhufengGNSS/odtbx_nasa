...
%   Given:
%
%      ck        the string scalar or NxM character array of SPICE CK 
%                file names 
%
%      idcode    the integer scalar NAIF ID code of an object, normally a 
...
%                window.
% 
%      level     the string scalar defining the level (granularity) at which 
...
%   the call:
%
%      cov = cspice_ckcov(ck, idcode, needav, level, tol, timsys, size, cover_i)
%
%         or
%
%      cov = cspice_ckcov(ck, idcode, needav, level, tol, timsys, size)
%
%   returns:
%
...
%            present in the file 'ck', The array 'cover' contains the pairs of
...
% 
%            When the coverage 'level' is "SEGMENT", 'cover' is computed in a 
...
%            result are merged. The resulting window is returned in 'cover'.
%            The expanded window in no case extends beyond the segment bounds
%            in either direction by more than 'tol'.
% 
%            The interval endpoints contained in 'cover' are encoded spacecraft
   ...
   switch nargin
      ...
      otherwise
         ...
         error ( ['Usage: [cov] = cspice_ckcov( _`ck`_, idcode, ' ...
                          'needav, level, tol, timsys, size, [cover_i])'] )
...
if ( nargin == 7 )
   ...
   %
   try
      cov = mice('ckcov_c', ck, idcode, needav, level, tol, timsys, size );
   catch
      rethrow(lasterror)
   end
