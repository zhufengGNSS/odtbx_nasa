...
/* Main program */ MAIN__(void)
{
    ...
    doublereal d__1;
    char ch__1[69], ch__2[2], ch__3[693];
    cllist cl__1;
    ...
    static char pridvl[80], indtvl[80], frnmvl[80], sgidvl[80], tmwrvl[80], 
	    appchr[80], delmvl[1];
    static integer outtvl, nlnrec, obidvl;
    extern logical odd_(integer *);
    static integer cnidvl, pldgvl, eplnvl, nflnvl;
    static doublereal cngmvl, cnervl[3];
    static integer inpunt, cmnunt;
    static logical eof;
    static integer jlepoc;
    static doublereal dstcof, tp[3], pv[3], dvl[50], timstp, requat;
    static logical statfl, stotfl, errflg;
    extern integer pos_(char *, char *, integer *, ftnlen, ftnlen);
    static logical appflg;
    ...
    extern /* Subroutine */ int errprt_(char *, char *, ftnlen, ftnlen), 
	    ...
	    spkopa_(char *, integer *, ftnlen), spkopn_(char *, char *, 
	    ...
	    ftnlen, ftnlen, ftnlen), dafcls_(integer *), delfil_(char *, 
	    ftnlen), redbuf_(integer *, char *, integer *, integer *, char *, 
	    integer *, char *, logical *, ftnlen, ftnlen, ftnlen), nparsd_(
	    char *, doublereal *, char *, integer *, ftnlen, ftnlen), setelm_(
	    doublereal *, doublereal *, doublereal *, doublereal *, integer *,
	     integer *, doublereal *, logical *, char *, ftnlen), conics_(
	    doublereal *, doublereal *, doublereal *), oscelt_(doublereal *, 
	    doublereal *, doublereal *, doublereal *), timout_(doublereal *, 
	    char *, char *, ftnlen, ftnlen), sxform_(char *, char *, 
	    doublereal *, doublereal *, ftnlen, ftnlen), bodmat_(integer *, 
	    doublereal *, doublereal *), recrad_(doublereal *, doublereal *, 
	    doublereal *, doublereal *), remlad_(integer *, integer *, 
	    doublereal *, integer *), spkcls_(integer *), chkout_(char *, 
...
/* Writing concatenation */
    i__1[0] = 57, a__1[0] = "MKSPK -- Version 5.1.0, March 6, 2009 -- Toolki"
	    "t Version ";
    i__1[1] = rtrim_(verstr, (ftnlen)12), a__1[1] = verstr;
    s_cat(ch__1, a__1, i__1, &c__2, (ftnlen)69);
    tostdo_(ch__1, rtrim_(verstr, (ftnlen)12) + 57);
    tostdo_(" ", (ftnlen)1);
...
/*        Let's see if they match. */
	...
	if (! iotype[(i__2 = itpidx + (otpidx << 2) - 5) < 32 && 0 <= i__2 ? 
		i__2 : s_rnge("iotype", i__2, "mkspk_", (ftnlen)941)]) {
...
/*     We have set of parameters that is enough to process SPK of */
/*     output type 10, but not for any other type. Continue collecting */
/*     setup information. */

    if (s_cmp(indtvl, "TL_ELEMENTS", rtrim_(indtvl, (ftnlen)80), (ftnlen)11) 
	    != 0) {

/*        Output SPK type is not 10. We continue to get setup */
/*        file parameters required for other output SPK types. */
...
/*                 Store line in the buffer. */
		    ...
		    s_copy(cmnbuf + ((i__2 = l + 5) < 506 && 0 <= i__2 ? i__2 
			    : s_rnge("cmnbuf", i__2, "mkspk_", (ftnlen)1616)) 
...
/*              Store line on buffer. */
		...
		++l;
		s_copy(cmnbuf + ((i__2 = l + 5) < 506 && 0 <= i__2 ? i__2 : 
			s_rnge("cmnbuf", i__2, "mkspk_", (ftnlen)1714)) * 255,
...
/*     Add "bottom of the comments" separator line. */
	...
	s_copy(cmnbuf + ((i__2 = l + 6) < 506 && 0 <= i__2 ? i__2 : s_rnge(
		"cmnbuf", i__2, "mkspk_", (ftnlen)1742)) * 255, " ", (ftnlen)
		255, (ftnlen)1);
	s_copy(cmnbuf + ((i__2 = l + 7) < 506 && 0 <= i__2 ? i__2 : s_rnge(
		"cmnbuf", i__2, "mkspk_", (ftnlen)1743)) * 255, astrln, (
...
/*        Process two-line element data and calculate */
/*        output SPK of type 10. */

	tle2spk_(inpfn, &obidvl, &cnidvl, frnmvl, sgidvl, &handle, (ftnlen)
		255, (ftnlen)80, (ftnlen)80);
    } else {
...
/*              Should we skip current value? */
		...
		if (param[(i__2 = n - 1) < 50 && 0 <= i__2 ? i__2 : s_rnge(
			"param", i__2, "mkspk_", (ftnlen)1879)] == 30) {
...
/*                    length" initial position. */
			...
			if (param[(i__2 = n - 1) < 50 && 0 <= i__2 ? i__2 : 
				s_rnge("param", i__2, "mkspk_", (ftnlen)1906)]
...
/*                    be present in each record in this situation. */
			...
			if (param[(i__2 = n - 1) < 50 && 0 <= i__2 ? i__2 : 
				s_rnge("param", i__2, "mkspk_", (ftnlen)1946)]
				 == 1) {
...
/*                 Is current value a time string or a number? */
		    ...
		    if (param[(i__2 = n - 1) < 50 && 0 <= i__2 ? i__2 : 
			    s_rnge("param", i__2, "mkspk_", (ftnlen)2047)] == 
...
/*                       Yes. Parse them as DPs. */
			    ...
			    nparsd_(wrkchr, &dvl[(i__3 = param[(i__2 = n - 1) 
				    < 50 && 0 <= i__2 ? i__2 : s_rnge("param",
				     i__2, "mkspk_", (ftnlen)2059)] - 1) < 50 
				    && 0 <= i__3 ? i__3 : s_rnge("dvl", i__3, 
				    "mkspk_", (ftnlen)2059)], error, &ptr, (
...
/*                          We can not parse this data value. */
				...
				errch_("#", doval + ((i__3 = param[(i__2 = n 
					...
					ftnlen)2078)] - 1) < 50 && 0 <= i__3 ?
					 i__3 : s_rnge("doval", i__3, "mkspk_"
					, (ftnlen)2078)) * 12, (ftnlen)1, (
					ftnlen)12);
...
/*                       pass it to the STR2ET. */
			    ...
			    str2et_(htime, &dvl[(i__3 = param[(i__2 = n - 1) <
				     50 && 0 <= i__2 ? i__2 : s_rnge("param", 
				    i__2, "mkspk_", (ftnlen)2089)] - 1) < 50 
				    && 0 <= i__3 ? i__3 : s_rnge("dvl", i__3, 
				    "mkspk_", (ftnlen)2089)], (ftnlen)512);
			}
		    } else {
...
/*                    Parse an input number as DP. */
			...
			nparsd_(wrkchr, &dvl[(i__3 = param[(i__2 = n - 1) < 
				50 && 0 <= i__2 ? i__2 : s_rnge("param", i__2,
				 "mkspk_", (ftnlen)2098)] - 1) < 50 && 0 <= 
				i__3 ? i__3 : s_rnge("dvl", i__3, "mkspk_", (
				ftnlen)2098)], error, &ptr, (ftnlen)512, (
				ftnlen)512);
			if (ptr != 0) {
...
/*                       We can not parse this data value. */
			    ...
			    errch_("#", doval + ((i__3 = param[(i__2 = n - 1) 
				    < 50 && 0 <= i__2 ? i__2 : s_rnge("param",
				     i__2, "mkspk_", (ftnlen)2116)] - 1) < 50 
				    && 0 <= i__3 ? i__3 : s_rnge("doval", 
				    i__3, "mkspk_", (ftnlen)2116)) * 12, (
				    ftnlen)1, (ftnlen)12);
			    sigerr_("SPICE(NOTADPNUMBER)", (ftnlen)19);
			}
		    }
...
/*                    Accumulate epoch in EPOCH buffer. */
			...
			epoch[(i__2 = j - 1) < 10000 && 0 <= i__2 ? i__2 : 
				s_rnge("epoch", i__2, "mkspk_", (ftnlen)2183)]
				 = dvl[0];
...
/*                    in STATE buffer . */
			...
			for (l = 1; l <= 6; ++l) {
			    state[(i__2 = l + j * 6 - 7) < 60000 && 0 <= i__2 
				    ...
				    ftnlen)2193)] = dvl[(i__3 = l) < 50 && 0 
				    <= i__3 ? i__3 : s_rnge("dvl", i__3, 
				    "mkspk_", (ftnlen)2193)] * dstcof;
			}

/*                    Accumulate epoch in EPOCH buffer. */

			epoch[(i__2 = j - 1) < 10000 && 0 <= i__2 ? i__2 : 
				s_rnge("epoch", i__2, "mkspk_", (ftnlen)2200)]
				 = dvl[0];
...
/*                    in the output SPK file. */
			...
			if (outtvl == 15 || outtvl == 17) {
			    oscelt_(&state[(i__2 = j * 6 - 6) < 60000 && 0 <= 
				    ...
				    "pk_", (ftnlen)2209)], &epoch[(i__3 = j - 
				    1) < 10000 && 0 <= i__3 ? i__3 : s_rnge(
				    "epoch", i__3, "mkspk_", (ftnlen)2209)], &
				    cngmvl, elts);
			}
		    } else {
...
/*                    parameters in the DVAL array. */
			...
			epoch[(i__2 = j - 1) < 10000 && 0 <= i__2 ? i__2 : 
				s_rnge("epoch", i__2, "mkspk_", (ftnlen)2221)]
				 = dvl[0];
		    }

/*                 Now we do additional processing depending on the */
/*                 output SPK type. There are two cases here: SPK */
/*                 type 15 for which we need to compute MEAN MOTION */
/*                 and SPK 8 and 12 for which we need to verify equal */
/*                 spacing between data points. */

		    if (outtvl == 15) {

/*                    We reduce input data to time of periapsis and */
...
/*                    Calculate state vector at epoch of periapsis. */
			...
			epoch[(i__2 = j - 1) < 10000 && 0 <= i__2 ? i__2 : 
				s_rnge("epoch", i__2, "mkspk_", (ftnlen)2267)]
				 = elts[6] - elts[5] / meanm;
			conics_(elts, &epoch[(i__2 = j - 1) < 10000 && 0 <= 
				i__2 ? i__2 : s_rnge("epoch", i__2, "mkspk_", 
				(ftnlen)2268)], &state[(i__3 = j * 6 - 6) < 
				60000 && 0 <= i__3 ? i__3 : s_rnge("state", 
				i__3, "mkspk_", (ftnlen)2268)]);
			elts[5] = 0.;
			elts[6] = epoch[(i__2 = j - 1) < 10000 && 0 <= i__2 ? 
...
/*                    must the same (within tolerance) as the one before. */
			...
			if (j > 2) {
			    tmpdp = epoch[(i__2 = j - 1) < 10000 && 0 <= i__2 
				    ? i__2 : s_rnge("epoch", i__2, "mkspk_", (
				    ftnlen)2282)] - epoch[(i__3 = j - 2) < 
				    10000 && 0 <= i__3 ? i__3 : s_rnge("epoch"
				    , i__3, "mkspk_", (ftnlen)2282)];
...
/*                    Memorize current time step. */
			...
			if (j > 1) {
			    tstep = epoch[(i__2 = j - 1) < 10000 && 0 <= i__2 
				    ? i__2 : s_rnge("epoch", i__2, "mkspk_", (
				    ftnlen)2322)] - epoch[(i__3 = j - 2) < 
				    10000 && 0 <= i__3 ? i__3 : s_rnge("epoch"
				    , i__3, "mkspk_", (ftnlen)2322)];
			}
		    }
...
/*                       reference frame at the time of our data point. */
			    ...
			    sxform_("J2000", frnmvl, &epoch[(i__2 = j - 1) < 
				    ...
				    , i__2, "mkspk_", (ftnlen)2349)], xfrm, (
				    ftnlen)5, (ftnlen)80);
			    for (l = 1; l <= 3; ++l) {
				for (m = 1; m <= 3; ++m) {
				    mwor[(i__2 = l + m * 3 - 4) < 9 && 0 <= 
					    i__2 ? i__2 : s_rnge("mwor", i__2,
					     "mkspk_", (ftnlen)2353)] = xfrm[(
					    i__3 = l + m * 6 - 7) < 36 && 0 <=
					     i__3 ? i__3 : s_rnge("xfrm", 
					    i__3, "mkspk_", (ftnlen)2353)];
				}
			    }
...
/*                    SPKWxx.) */
			...
			if (j > 1) {
			    if (epoch[(i__2 = j - 1) < 10000 && 0 <= i__2 ? 
				    i__2 : s_rnge("epoch", i__2, "mkspk_", (
				    ftnlen)2544)] == epoch[(i__3 = j - 2) < 
				    10000 && 0 <= i__3 ? i__3 : s_rnge("epoch"
				    , i__3, "mkspk_", (ftnlen)2544)]) {
				dafcls_(&handle);
				delfil_(outfn, (ftnlen)255);
				timout_(&epoch[(i__2 = j - 1) < 10000 && 0 <= 
					i__2 ? i__2 : s_rnge("epoch", i__2, 
					"mkspk_", (ftnlen)2547)], "YYYY-MM-D"
					"DTHR:MN:SC.### ::RND", error, (ftnlen)
					29, (ftnlen)512);
			    ...
			    if (epoch[(i__2 = j - 1) < 10000 && 0 <= i__2 ? 
				    i__2 : s_rnge("epoch", i__2, "mkspk_", (
				    ftnlen)2560)] < epoch[(i__3 = j - 2) < 
				    10000 && 0 <= i__3 ? i__3 : s_rnge("epoch"
				    , i__3, "mkspk_", (ftnlen)2560)]) {
				dafcls_(&handle);
				delfil_(outfn, (ftnlen)255);
				setmsg_("The epoch of the input data record "
...
/*           types 8, 9, 12 and 13. */
		...
		if (statfl && epoch[(i__2 = j - 1) < 10000 && 0 <= i__2 ? 
			i__2 : s_rnge("epoch", i__2, "mkspk_", (ftnlen)2982)] 
			< etbeg) {
...
/*                 than START time defined in setup. Complain. */
		...
		if (stotfl && epoch[(i__2 = j - 1) < 10000 && 0 <= i__2 ? 
			i__2 : s_rnge("epoch", i__2, "mkspk_", (ftnlen)2999)] 
			< etend) {
...
/*           only one state, it's zero. */
		...
		timstp = (epoch[(i__2 = jlepoc - 1) < 10000 && 0 <= i__2 ? 
			i__2 : s_rnge("epoch", i__2, "mkspk_", (ftnlen)3030)] 
