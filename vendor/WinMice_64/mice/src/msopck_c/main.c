...
static doublereal c_b590 = 1.;
static integer c__4 = 4;
static doublereal c_b935 = .5;
...
/* Main program */ MAIN__(void)
{
    ...
    extern /* Subroutine */ int errdp_(char *, doublereal *, ftnlen), repmi_(
    ...
    extern /* Subroutine */ int xpose_(doublereal *, doublereal *), repmf_(
	    char *, char *, doublereal *, integer *, char *, char *, ftnlen, 
    ...
    extern /* Subroutine */ int ck3sdn_(doublereal *, logical *, integer *, 
	    doublereal *, doublereal *, doublereal *, integer *, doublereal *,
	     doublereal *, integer *);
    static logical found1, found2, addtab;
    ...
    static doublereal offang[3], qn[4], earate, coefft;
    static char fsclkf[265], dashln[80];
    static doublereal eulang[3];
    static char templt[80*48], prodid[265], cmmntf[265], inputf[265], linesv[
	    265], outptf[265], setupf[265], frmnam[40], astrln[80], offaxs[1*
	    3];
    static doublereal intrvl, offmat[9]	/* was [3][3] */, qerror, rerror[3], 
	    startt[100000], hdparr[100000], sclkdp, nticks, begtim, endtim, 
	    sstrtt, timcor, sdntol, sclkmd[2], sclkof[2], clklft, clkrgh, 
	    clkfrc, curmat[9]	/* was [3][3] */, hdp, prvmat[9]	/* 
	    was [3][3] */, nxtmat[9]	/* was [3][3] */, scldav[3], tmpvec[3]
	    , tmpmat[9]	/* was [3][3] */, mat[9]	/* was [3][3] */;
    static integer eulaxs[3], offaxi[3], iorder[100000], cktype, inplun, 
	    instid, lcount, scrtch, srtidx, stpidx, nfilds, nitems;
    static logical angrat, insarf, appndf, eof, ckopnd, badrat;
    static integer ptr;
    static logical seopnd, siopnd, cmmflg, offrot, savbtm, muarat, avgrat, 
	    wrtseg, eulbod, dnsmpl;
...
/*                                    area of the ouput CK and program's */
...
/*              components is now now compared to the user supplied */
/*              maximum; */
...
/*        formats. Added setup file keyword controling inclusion of the */
...
/*     Increment, as fraction of a tick, to added to the start time */
/*     of the "zero" length intervals in type 2 CK with made up */
/*     angular rates. */
...
/*     un-initialized. All of these are initialized within milti-branch */
...
/* Writing concatenation */
    i__1[0] = 70, a__1[0] = "MSOPCK Utility Program, Version 6.0.0, 2006-11-"
	    "07; SPICE Toolkit Ver. ";
...
/*        not, display usage. */
	...
	if (pos_(line, " -H ", &c__1, (ftnlen)265, (ftnlen)4) != 0 || pos_(
	    ...
	    for (i__ = 1; i__ <= 6; ++i__) {
		tostdo_(templt + ((i__2 = i__ - 1) < 48 && 0 <= i__2 ? i__2 : 
			s_rnge("templt", i__2, "msopck_", (ftnlen)893)) * 80, 
			(ftnlen)80);
	    }
	    ...
	    s_copy(templt + 3440, " ", (ftnlen)80, (ftnlen)1);
	    s_copy(templt + 3520, "      PRODUCER_ID             = 'producer"
		    " group/person name'", (ftnlen)80, (ftnlen)60);
	    s_copy(templt + 3600, " ", (ftnlen)80, (ftnlen)1);
	    s_copy(templt + 3680, "   \\begintext", (ftnlen)80, (ftnlen)13);
	    s_copy(templt + 3760, " ", (ftnlen)80, (ftnlen)1);
	    for (i__ = 1; i__ <= 48; ++i__) {
		tostdo_(templt + ((i__2 = i__ - 1) < 48 && 0 <= i__2 ? i__2 : 
			s_rnge("templt", i__2, "msopck_", (ftnlen)986)) * 80, 
			(ftnlen)80);
	    }
	} else {
...
/* Writing concatenation */
	    ...
	    mkfclk_(fsclkf, &scid, &c_b141, ch__2, (ftnlen)265, (ftnlen)286);
...
/*           check them. */
		...
		if (rerror[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : s_rnge(
			"rerror", i__2, "msopck_", (ftnlen)1422)] <= 0.) {
...
/*           If it's integer we just check it and save it. */
		...
		for (i__ = 1; i__ <= 3; ++i__) {
			...
			eulaxs[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : 
			    ...
			    ftnlen)1745)), "Y", (ftnlen)1, (ftnlen)1)) {
			eulaxs[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : 
				s_rnge("eulaxs", i__2, "msopck_", (ftnlen)
				1746)] = 2;
		    } else if (eqstr_(eaxis + ((i__2 = i__ - 1) < 3 && 0 <= 
			    i__2 ? i__2 : s_rnge("eaxis", i__2, "msopck_", (
			    ftnlen)1747)), "Z", (ftnlen)1, (ftnlen)1)) {
			eulaxs[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : 
				s_rnge("eulaxs", i__2, "msopck_", (ftnlen)
				1748)] = 3;
		    } else {
		...
		for (i__ = 1; i__ <= 3; ++i__) {
		    if (eulaxs[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : 
			    s_rnge("eulaxs", i__2, "msopck_", (ftnlen)1762)] 
...
/*           it and save it. */
		...
		for (i__ = 1; i__ <= 3; ++i__) {
			...
			offaxi[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : 
			    ...
			    ftnlen)1898)), "Y", (ftnlen)1, (ftnlen)1)) {
			offaxi[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : 
				s_rnge("offaxi", i__2, "msopck_", (ftnlen)
				1899)] = 2;
		    } else if (eqstr_(offaxs + ((i__2 = i__ - 1) < 3 && 0 <= 
			    i__2 ? i__2 : s_rnge("offaxs", i__2, "msopck_", (
			    ftnlen)1900)), "Z", (ftnlen)1, (ftnlen)1)) {
			offaxi[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : 
				s_rnge("offaxi", i__2, "msopck_", (ftnlen)
				1901)] = 3;
		    } else {
		...
		for (i__ = 1; i__ <= 3; ++i__) {
		    if (offaxi[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : 
			    s_rnge("offaxi", i__2, "msopck_", (ftnlen)1915)] 
...
/*              Go from degrees to radians. */
		...
		for (i__ = 1; i__ <= 3; ++i__) {
		    offang[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : s_rnge(
			    "offang", i__2, "msopck_", (ftnlen)1960)] = 
			    offang[(i__3 = i__ - 1) < 3 && 0 <= i__3 ? i__3 : 
			    s_rnge("offang", i__3, "msopck_", (ftnlen)1960)] *
			     rpd_();
		}
	    } else if (eqstr_(word, "RADIANS", (ftnlen)40, (ftnlen)7)) {
...
/*        Nop, there were no offset Euler angles. Set corresponding flag. */
...
/*           SCLKs right before writing CK file. */
		...
		nparsd_(word, &startt[(i__2 = index - 1) < 100000 && 0 <= 
			i__2 ? i__2 : s_rnge("startt", i__2, "msopck_", (
			ftnlen)2164)], error, &ptr, (ftnlen)40, (ftnlen)265);
		if (ptr != 0) {
...
/*              while saving fractional ticks. */
		...
		rmaind_(&sclkdp, &c_b590, &clklft, &clkfrc);
		clkfrc *= sclkmd[1];
		rmaind_(&clkfrc, &c_b590, &clkrgh, &tmpdp);
		clkfrc = tmpdp;
		clkrgh += sclkof[1];
...
/*           Add time bias (TIMCOR is 0 if no correction was requested.) */
	    ...
	    startt[(i__2 = index - 1) < 100000 && 0 <= i__2 ? i__2 : s_rnge(
		    "startt", i__2, "msopck_", (ftnlen)2219)] = startt[(i__3 =
		     index - 1) < 100000 && 0 <= i__3 ? i__3 : s_rnge("startt"
		    , i__3, "msopck_", (ftnlen)2219)] + timcor;
...
/*           second time tag. */
		...
		if (eqstr_(ttype, "SCLK", (ftnlen)40, (ftnlen)4)) {
		    ...
		    nparsd_(word, &stopt[(i__2 = index - 1) < 100000 && 0 <= 
			    i__2 ? i__2 : s_rnge("stopt", i__2, "msopck_", (
			    ftnlen)2257)], error, &ptr, (ftnlen)40, (ftnlen)
			    265);
		    if (ptr != 0) {
...
/*              requested.) */
		...
		stopt[(i__2 = index - 1) < 100000 && 0 <= i__2 ? i__2 : 
			s_rnge("stopt", i__2, "msopck_", (ftnlen)2314)] = 
			stopt[(i__3 = index - 1) < 100000 && 0 <= i__3 ? i__3 
			: s_rnge("stopt", i__3, "msopck_", (ftnlen)2314)] + 
			timcor;
	    }
...
/*              intermediate quaternion. */
		...
		for (i__ = 1; i__ <= 4; ++i__) {
		    ...
		    nparsd_(word, &hquat[(i__2 = i__ - 1) < 4 && 0 <= i__2 ? 
			    ...
			    2331)], error, &ptr, (ftnlen)40, (ftnlen)265);
		    if (ptr != 0) {
...
/*              conjugate (shift/negate) it along the way. */
		...
		quats[(i__2 = (index << 2) - 4) < 400000 && 0 <= i__2 ? i__2 :
			 s_rnge("quats", i__2, "msopck_", (ftnlen)2345)] = 
			hquat[3];
		quats[(i__2 = (index << 2) - 3) < 400000 && 0 <= i__2 ? i__2 :
			 s_rnge("quats", i__2, "msopck_", (ftnlen)2346)] = 
			-hquat[0];
		quats[(i__2 = (index << 2) - 2) < 400000 && 0 <= i__2 ? i__2 :
			 s_rnge("quats", i__2, "msopck_", (ftnlen)2347)] = 
			-hquat[1];
		quats[(i__2 = (index << 2) - 1) < 400000 && 0 <= i__2 ? i__2 :
			 s_rnge("quats", i__2, "msopck_", (ftnlen)2348)] = 
			-hquat[2];
		...
		for (i__ = 1; i__ <= 4; ++i__) {
		    ...
		    nparsd_(word, &quats[(i__2 = i__ + (index << 2) - 5) < 
			    400000 && 0 <= i__2 ? i__2 : s_rnge("quats", i__2,
			     "msopck_", (ftnlen)2357)], error, &ptr, (ftnlen)
			    40, (ftnlen)265);
		    if (ptr != 0) {
			setmsg_("The quaternion component '#' from the line "
				"# of the input file is not a number.", (
...
/*              store quaternion in the main buffer. */
		...
		for (i__ = 1; i__ <= 3; ++i__) {
		    ...
		    nparsd_(word, &eulang[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? 
			    ...
			    2376)], error, &ptr, (ftnlen)40, (ftnlen)265);
		    if (ptr != 0) {
...
/*              Convert the matrix to quaterions. */
...
/*              matrices to be provided in row-major order. */
		...
		for (i__ = 1; i__ <= 3; ++i__) {
			...
			nparsd_(word, &mat[(i__2 = i__ + j * 3 - 4) < 9 && 0 
				...
				, (ftnlen)2429)], error, &ptr, (ftnlen)40, (
				ftnlen)265);
			if (ptr != 0) {
...
/*              representation -- we should always have 3 elements. */
		...
		for (i__ = 1; i__ <= 3; ++i__) {
		    ...
		    nparsd_(word, &avvs[(i__2 = i__ + index * 3 - 4) < 300000 
			    ...
			    "ck_", (ftnlen)2457)], error, &ptr, (ftnlen)40, (
			    ftnlen)265);
		    if (ptr != 0) {
...
/*                 angles. */
			...
			avvs[(i__2 = i__ + index * 3 - 4) < 300000 && 0 <= 
				...
				(ftnlen)2473)] = avvs[(i__3 = i__ + index * 3 
				- 4) < 300000 && 0 <= i__3 ? i__3 : s_rnge(
				"avvs", i__3, "msopck_", (ftnlen)2473)] * 
				earate;
		    }
		}
	    }
...
/*              comparison of each element with it's normalized element. */
		...
		for (j = 1; j <= 4; ++j) {
		    if ((d__1 = quats[(i__2 = j + (index << 2) - 5) < 400000 
			    ...
			    "pck_", (ftnlen)2508)] - qn[(i__3 = j - 1) < 4 && 
			    0 <= i__3 ? i__3 : s_rnge("qn", i__3, "msopck_", (
			    ftnlen)2508)], abs(d__1)) > qerror) {
			badqua = TRUE_;
		    }
		}
...
/*           Reorder quats column by column. */
		...
		for (j = 1; j <= i__2; ++j) {
		    hdparr[(i__3 = j - 1) < 100000 && 0 <= i__3 ? i__3 : 
			    s_rnge("hdparr", i__3, "msopck_", (ftnlen)2711)] =
...
/*           Do the same for AVVS. */
		...
		for (j = 1; j <= i__2; ++j) {
		    hdparr[(i__3 = j - 1) < 100000 && 0 <= i__3 ? i__3 : 
			    s_rnge("hdparr", i__3, "msopck_", (ftnlen)2724)] =
...
/*           such.) */
	    ...
	    sstrtt = startt[(i__2 = index - 1) < 100000 && 0 <= i__2 ? i__2 : 
		    s_rnge("startt", i__2, "msopck_", (ftnlen)2737)];
	    sstpt = stopt[(i__2 = index - 1) < 100000 && 0 <= i__2 ? i__2 : 
		    s_rnge("stopt", i__2, "msopck_", (ftnlen)2738)];
	    squat[0] = quats[(i__2 = (index << 2) - 4) < 400000 && 0 <= i__2 ?
		     i__2 : s_rnge("quats", i__2, "msopck_", (ftnlen)2740)];
	    squat[1] = quats[(i__2 = (index << 2) - 3) < 400000 && 0 <= i__2 ?
		     i__2 : s_rnge("quats", i__2, "msopck_", (ftnlen)2741)];
	    squat[2] = quats[(i__2 = (index << 2) - 2) < 400000 && 0 <= i__2 ?
		     i__2 : s_rnge("quats", i__2, "msopck_", (ftnlen)2742)];
	    squat[3] = quats[(i__2 = (index << 2) - 1) < 400000 && 0 <= i__2 ?
		     i__2 : s_rnge("quats", i__2, "msopck_", (ftnlen)2743)];
	    savv[0] = avvs[(i__2 = index * 3 - 3) < 300000 && 0 <= i__2 ? 
		    i__2 : s_rnge("avvs", i__2, "msopck_", (ftnlen)2745)];
	    savv[1] = avvs[(i__2 = index * 3 - 2) < 300000 && 0 <= i__2 ? 
		    i__2 : s_rnge("avvs", i__2, "msopck_", (ftnlen)2746)];
	    savv[2] = avvs[(i__2 = index * 3 - 1) < 300000 && 0 <= i__2 ? 
		    i__2 : s_rnge("avvs", i__2, "msopck_", (ftnlen)2747)];
...
/*           Write segment interval table header. */
		...
		timout_(&stopt[(i__2 = index - 1) < 100000 && 0 <= i__2 ? 
			i__2 : s_rnge("stopt", i__2, "msopck_", (ftnlen)2769)]
			, "YYYY-MM-DDTHR:MN:SC.###", hword, (ftnlen)23, (
			ftnlen)40);
	    } else {
		timout_(&startt[(i__2 = index - 1) < 100000 && 0 <= i__2 ? 
			i__2 : s_rnge("startt", i__2, "msopck_", (ftnlen)2771)
			], "YYYY-MM-DDTHR:MN:SC.###", hword, (ftnlen)23, (
			ftnlen)40);
	    }
	    repmc_(hline, "#", hword, hline, (ftnlen)265, (ftnlen)1, (ftnlen)
...
/*              We just need to convert times to encoded SCLKs. */
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
		    ...
		    startt[(i__3 = i__ - 1) < 100000 && 0 <= i__3 ? i__3 : 
			    s_rnge("startt", i__3, "msopck_", (ftnlen)2790)] =
			     tmpdp;
		}
...
/*              For type 2 we don't have SCLK rates. Too bad. We need */
/*              to go and get them from the SCLK values loaded into the */
/*              kernel pool. We need two variables from the POOL: */
/*              SCLK01_MODULI_XXX and SCLK01_COEFFICIENTS_XXX. From */
/*              MODULI we will get a number of SCLK ticks per in one */
/*              left-most field count, from COEFFICIENTS we will get */
/*              number of TDB/TDB seconds in one left-most count. By */
/*              diving second number by first number we will compute */
/*              desired rate. */

		s_copy(word, "SCLK01_MODULI_#", (ftnlen)40, (ftnlen)15);
		i__2 = abs(scid);
		repmi_(word, "#", &i__2, word, (ftnlen)40, (ftnlen)1, (ftnlen)
			40);
		dtpool_(word, &found, &n, hword, (ftnlen)40, (ftnlen)40);
		if (! found || n < 1 || s_cmp(hword, "N", (ftnlen)40, (ftnlen)
			1) != 0) {
		    setmsg_("Cannot find the SCLK keyword '#' in the POOL or"
			    " the keyword typeand/or the number of values don"
			    "'t match expectations.", (ftnlen)117);
		    errch_("#", word, (ftnlen)1, (ftnlen)40);
		    sigerr_("SPICE(SCLKMODULINOTFOUND)", (ftnlen)25);
		}

/*              We did find MODULI value. Let's compute number of */
/*              ticks in the leftmost field. */

		nticks = 1.;
		if (n > 1) {
		    i__2 = n;
		    for (i__ = 2; i__ <= i__2; ++i__) {
			gdpool_(word, &i__, &c__1, &j, &hdp, &found, (ftnlen)
				40);
			nticks *= hdp;
		    }
		}

/*              Now get coefficient value. */

		s_copy(word, "SCLK01_COEFFICIENTS_#", (ftnlen)40, (ftnlen)21);
		i__2 = abs(scid);
		repmi_(word, "#", &i__2, word, (ftnlen)40, (ftnlen)1, (ftnlen)
			40);
		dtpool_(word, &found, &n, hword, (ftnlen)40, (ftnlen)40);
		if (! found || n < 3 || s_cmp(hword, "N", (ftnlen)40, (ftnlen)
			1) != 0) {
		    setmsg_("Cannot find SCLK keyword '#' in the POOL or the"
			    " keyword type and/or the number of values don't "
			    "match expectations.", (ftnlen)114);
		    errch_("#", word, (ftnlen)1, (ftnlen)40);
		    sigerr_("SPICE(SCLKCOEFFSNOTFOUND)", (ftnlen)25);
		}

/*              We did find COEFF value. Let's compute average */
/*              coefficient and use. Well, this is a BIG stretch -- it */
/*              could be that SLCK that we have got was generated by "a */
/*              donkey" and coefficient values are jumping all round. */
/*              But we don't have time be sophisticated and, so, will */
/*              hope for the best for now. */
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
		    i__3 = i__ * 3;
		    gdpool_(word, &i__3, &c__1, &j, &hdpa, &found, (ftnlen)40)
			    ;
		    hdp += hdpa;
		}
		coefft = hdp / (doublereal) (n / 3);

/*              Adjust COEFFT to be number of secs in tick rather */
/*              than a number of secs in left most field. */

		if (nticks == 0.) {
		    setmsg_("It looks like the number of TICKS in the left-m"
			    "ost SCLK field is zero. This can happen only if "
			    "the SCLK file had zero as one of the elements in"
			    " the MODULI keyword.", (ftnlen)163);
		    sigerr_("SPICE(BADSCLKDATA)", (ftnlen)18);
		} else {
		    coefft /= nticks;
		}
...
/*              Assign computed rates. */
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
		    rates[(i__3 = i__ - 1) < 100000 && 0 <= i__3 ? i__3 : 
			    s_rnge("rates", i__3, "msopck_", (ftnlen)2909)] = 
			    coefft;
		}
...
/*                       the next point. */
			    ...
			    if (startt[(i__3 = i__) < 100000 && 0 <= i__3 ? 
				    ...
				    ftnlen)2948)] - startt[(i__4 = i__ - 1) < 
				    100000 && 0 <= i__4 ? i__4 : s_rnge("sta"
				    "rtt", i__4, "msopck_", (ftnlen)2948)] <= 
				    intrvl) {
...
/*                          duration, if it's non-zero. */
				...
				linrot_m__(curmat, nxtmat, &c_b590, hmat, 
					scldav);
				if (startt[(i__3 = i__) < 100000 && 0 <= i__3 
					? i__3 : s_rnge("startt", i__3, "mso"
					"pck_", (ftnlen)2975)] - startt[(i__4 =
					 i__ - 1) < 100000 && 0 <= i__4 ? 
				    ...
				    stopt[(i__3 = i__ - 1) < 100000 && 0 <= 
					    ...
					    i__3, "msopck_", (ftnlen)2979)] = 
					    startt[(i__4 = i__) < 100000 && 0 
					    <= i__4 ? i__4 : s_rnge("startt", 
					    i__4, "msopck_", (ftnlen)2979)];
				} else {
...
/*              Generate comment area intervals table. */
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
			...
			timout_(&stopt[(i__3 = i__ - 1) < 100000 && 0 <= i__3 
				...
				ftnlen)3053)], "YYYY-MM-DDTHR:MN:SC.###", 
			...
			if (stopt[(i__3 = i__ - 1) < 100000 && 0 <= i__3 ? 
				i__3 : s_rnge("stopt", i__3, "msopck_", (
				ftnlen)3060)] < startt[(i__4 = i__) < 100000 
			    ...
			    timout_(&stopt[(i__3 = i__ - 1) < 100000 && 0 <= 
				    ...
				    "pck_", (ftnlen)3066)], "YYYY-MM-DDTHR:MN"
...
/*              intervals table in the comment area. */
		...
		rates[(i__2 = nints - 1) < 100000 && 0 <= i__2 ? i__2 : 
			s_rnge("rates", i__2, "msopck_", (ftnlen)3101)] = 
			startt[0];
		if (index > 1) {
		    i__2 = index;
		    for (i__ = 2; i__ <= i__2; ++i__) {
			if (startt[(i__3 = i__ - 1) < 100000 && 0 <= i__3 ? 
				i__3 : s_rnge("startt", i__3, "msopck_", (
				ftnlen)3105)] - startt[(i__4 = i__ - 2) < 
				100000 && 0 <= i__4 ? i__4 : s_rnge("startt", 
				i__4, "msopck_", (ftnlen)3105)] > intrvl) {
			    ++nints;
			    rates[(i__3 = nints - 1) < 100000 && 0 <= i__3 ? 
				    i__3 : s_rnge("rates", i__3, "msopck_", (
				    ftnlen)3107)] = startt[(i__4 = i__ - 1) < 
				    100000 && 0 <= i__4 ? i__4 : s_rnge("sta"
				    "rtt", i__4, "msopck_", (ftnlen)3107)];
			    stopt[(i__3 = nints - 2) < 100000 && 0 <= i__3 ? 
				    i__3 : s_rnge("stopt", i__3, "msopck_", (
				    ftnlen)3108)] = startt[(i__4 = i__ - 2) < 
				    100000 && 0 <= i__4 ? i__4 : s_rnge("sta"
				    "rtt", i__4, "msopck_", (ftnlen)3108)];
			}
		    }
		}
		stopt[(i__2 = nints - 1) < 100000 && 0 <= i__2 ? i__2 : 
			s_rnge("stopt", i__2, "msopck_", (ftnlen)3112)] = 
			startt[(i__3 = index - 1) < 100000 && 0 <= i__3 ? 
			i__3 : s_rnge("startt", i__3, "msopck_", (ftnlen)3112)
			];
...
/*                       between them. */
			    ...
			    vhatg_(&quats[(i__3 = (i__ << 2) - 4) < 400000 && 
				    ...
				    "msopck_", (ftnlen)3149)], &c__4, qn);
			    q2m_(qn, curmat);
			    linrot_m__(prvmat, curmat, &c_b590, hmat, scldav);
			    if (startt[(i__3 = i__ - 1) < 100000 && 0 <= i__3 
				    ? i__3 : s_rnge("startt", i__3, "msopck_",
				     (ftnlen)3154)] - startt[(i__4 = i__ - 2) 
				    < 100000 && 0 <= i__4 ? i__4 : s_rnge(
				    "startt", i__4, "msopck_", (ftnlen)3154)] 
				    != 0.) {
				hrate = 1. / (startt[(i__3 = i__ - 1) < 
					100000 && 0 <= i__3 ? i__3 : s_rnge(
					"startt", i__3, "msopck_", (ftnlen)
					3156)] - startt[(i__4 = i__ - 2) < 
					100000 && 0 <= i__4 ? i__4 : s_rnge(
...
/*                       start and stop time arrays. */
			    ...
			    srtidx = isrchd_(&startt[(i__3 = i__ - 2) < 
				    ...
				    "rtt", i__3, "msopck_", (ftnlen)3179)], &
				    nints, rates);
			    stpidx = isrchd_(&startt[(i__3 = i__ - 2) < 
				    100000 && 0 <= i__3 ? i__3 : s_rnge("sta"
				    "rtt", i__3, "msopck_", (ftnlen)3180)], &
				    nints, stopt);
...
/*                          for it is to set its rate to zero. */
				...
				avvs[(i__3 = (i__ - 1) * 3 - 3) < 300000 && 0 
					<= i__3 ? i__3 : s_rnge("avvs", i__3, 
					"msopck_", (ftnlen)3190)] = 0.;
				avvs[(i__3 = (i__ - 1) * 3 - 2) < 300000 && 0 
					<= i__3 ? i__3 : s_rnge("avvs", i__3, 
					"msopck_", (ftnlen)3191)] = 0.;
				avvs[(i__3 = (i__ - 1) * 3 - 1) < 300000 && 0 
					<= i__3 ? i__3 : s_rnge("avvs", i__3, 
					"msopck_", (ftnlen)3192)] = 0.;
			    } else if (srtidx != 0) {
...
/*                          point. */
				...
				vequ_(&avvs[(i__3 = i__ * 3 - 3) < 300000 && 
					...
					i__3, "msopck_", (ftnlen)3203)], &
					avvs[(i__4 = (i__ - 1) * 3 - 3) < 
					300000 && 0 <= i__4 ? i__4 : s_rnge(
					"avvs", i__4, "msopck_", (ftnlen)3203)
					]);
			    } else if (stpidx != 0) {
...
/*                             point. */
				    ...
				    vadd_(&avvs[(i__3 = (i__ - 1) * 3 - 3) < 
					    300000 && 0 <= i__3 ? i__3 : 
					    s_rnge("avvs", i__3, "msopck_", (
					    ftnlen)3237)], &avvs[(i__4 = i__ *
					     3 - 3) < 300000 && 0 <= i__4 ? 
					    i__4 : s_rnge("avvs", i__4, "mso"
					    "pck_", (ftnlen)3237)], tmpvec);
				    vscl_(&c_b935, tmpvec, &avvs[(i__3 = (i__ 
					    - 1) * 3 - 3) < 300000 && 0 <= 
					    i__3 ? i__3 : s_rnge("avvs", i__3,
					     "msopck_", (ftnlen)3238)]);
				} else {
...
/*                    which is there is OK already. */
			...
			if (startt[(i__2 = index - 1) < 100000 && 0 <= i__2 ? 
				...
				ftnlen)3270)] == rates[(i__3 = nints - 1) < 
				100000 && 0 <= i__3 ? i__3 : s_rnge("rates", 
				i__3, "msopck_", (ftnlen)3270)]) {
...
/*              Convert ETs to SCLKs for start times. */
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
		    sce2c_(&scid, &startt[(i__3 = i__ - 1) < 100000 && 0 <= 
			    ...
			    ftnlen)3295)], &tmpdp);
		    startt[(i__3 = i__ - 1) < 100000 && 0 <= i__3 ? i__3 : 
			    s_rnge("startt", i__3, "msopck_", (ftnlen)3296)] =
			     tmpdp;
		}
...
/*              Now (at last!) convert interval start times to SCLKs. */
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
		    sce2c_(&scid, &rates[(i__3 = i__ - 1) < 100000 && 0 <= 
			    ...
			    ftnlen)3315)], &tmpdp);
		    rates[(i__3 = i__ - 1) < 100000 && 0 <= i__3 ? i__3 : 
			    s_rnge("rates", i__3, "msopck_", (ftnlen)3316)] = 
			    tmpdp;
		}
	    }
...
/*           comment area meta-information output. */
		...
		ckw03_(&handle, startt, &startt[(i__2 = index - 1) < 100000 &&
			 0 <= i__2 ? i__2 : s_rnge("startt", i__2, "msopck_", 
			(ftnlen)3390)], &instid, frmnam, &arflag, segid, &
			index, startt, quats, avvs, &nints, rates, (ftnlen)40,
			 (ftnlen)40);
		endtim = startt[(i__2 = index - 1) < 100000 && 0 <= i__2 ? 
			i__2 : s_rnge("startt", i__2, "msopck_", (ftnlen)3395)
			];
	    }
