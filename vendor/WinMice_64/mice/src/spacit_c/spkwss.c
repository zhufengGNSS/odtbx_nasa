	 ...
	 doublereal *segetm, ftnlen segid_len)
{
    ...
    static char spktyp[80*18] = "Modified Difference Array                  "
...
/*     Format the SPK segment type and a description if we have one. */
    ...
    if (*segtyp > 18 || *segtyp < 1) {
	s_copy(typdsc, "No description for this type. Do you need a new tool"
		"kit?", (ftnlen)80, (ftnlen)56);
    } else {
	s_copy(typdsc, spktyp + ((i__1 = *segtyp - 1) < 18 && 0 <= i__1 ? 
		i__1 : s_rnge("spktyp", i__1, "spkwss_", (ftnlen)388)) * 80, (
		ftnlen)80, (ftnlen)80);
    }
