...
/* Main program */ MAIN__(void)
{
    ...
    integer idxext, dotpos, opnset[7];
...
/*     appropriate filename extension, based on the filename extension of */
/*     the binary file. If TOXFR does not recognize the filename */
/*     extension of the input binary file, or there is no filename */
/*     extension on the input binary file, a filename extension of '.xfr' */
/*     will be used as the filename extension of the output transfer */
/*     file. */
...
/*     below and converts them into the corresponding filename extensions */
...
/* Writing concatenation */
	...
	if (dotpos > 0) {
		...
		s_copy(extnsn, xfrext + ((i__2 = idxext - 1) < 4 && 0 <= i__2 
			? i__2 : s_rnge("xfrext", i__2, "toxfr_", (ftnlen)270)
			) * 3, (ftnlen)3, (ftnlen)3);
...
/*        open and close them. */
	...
	if (numopn > 0) {
		...
		dafcls_(&opnset[(i__3 = i__ + 5) < 7 && 0 <= i__3 ? i__3 : 
			s_rnge("opnset", i__3, "toxfr_", (ftnlen)332)]);
	    }
	}
...
/*        open and close them. */
	...
	if (numopn > 0) {
		...
		dascls_(&opnset[(i__3 = i__ + 5) < 7 && 0 <= i__3 ? i__3 : 
			s_rnge("opnset", i__3, "toxfr_", (ftnlen)351)]);
	    }
	}
	if (exists_(xfrfnm, (ftnlen)128)) {
